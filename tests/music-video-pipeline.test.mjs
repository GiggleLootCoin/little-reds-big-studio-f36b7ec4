...
test("music video engines point at currently published free Spaces", () => {
  assert.deepEqual(MUSIC_VIDEO_ENGINE_SPACES, ["abidlabs/MiniMax-H3-Turbo-Lora", "KiroKusA/video-gen-ui"]);
});
test("music video planner covers the complete song with bounded generation chunks", () => { const plan = buildMusicVideoPlan({ durationSeconds: 187, sceneSeconds: 12 }); assert.equal(plan.totalDurationSeconds, 187); assert.equal(plan.chunks.at(-1)?.endSeconds, 187); assert.ok(plan.chunks.length >= 15); assert.ok(plan.chunks.every((chunk) => chunk.durationSeconds >= 2)); assert.ok(plan.chunks.every((chunk) => chunk.durationSeconds <= 14)); });
