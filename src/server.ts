...
function chatText(result: unknown): string {
  ...
  if (result && typeof result === "object") for (const key of ["response", "text", "generated_text", "output", "content"]) { const value = (result as Record<string, unknown>)[key]; if (typeof value === "string" && value.trim()) return value.trim(); }
  return "";
}
async function cloudflareAI(request: Request, env: ServerEnv): Promise<Response | null> {
  ...
  let body: { capability?: string; prompt?: string; language?: string; messages?: unknown[]; speaker?: string; audioBase64?: string };
  try { body = await request.json(); } catch { return jsonError("Invalid JSON request.", 400); }
  const capability = body.capability ?? ""; const prompt = String(body.prompt ?? "").trim();
  ...
  try {
    if (capability === "speech-to-text") {
      ...
      const result = await env.AI.run("@cf/openai/whisper", audio); return Response.json(result, { headers: { "cache-control": "no-store" } });
    }
    if (capability === "image") {
      // FLUX.2 Klein 4B requires multipart input when called through the binding.
      ...
      const image = await rawImageResponse(result); if (!image) return jsonError("Image model returned no usable image."); return image;
    }
    if (capability === "tts") {
      try {
        const result = await env.AI.run("@cf/myshell-ai/melotts", { prompt, lang: body.language || "en" });
        const audio = await rawAudioResponse(result); if (audio) return audio;
      } catch (error) { if (!isCapacityError(error)) console.warn("MeloTTS failed; trying Aura-2", error); }
      try {
        const aura = await env.AI.run("@cf/deepgram/aura-2-en", { text: prompt, encoding: "mp3", speaker: body.speaker || "luna" }, { returnRawResponse: true });
        const audio = await rawAudioResponse(aura); if (audio) return audio;
      } catch (error) { console.warn("Aura-2 failed; trying Aura-1", error); }
      const aura1 = await env.AI.run("@cf/deepgram/aura-1", { text: prompt, encoding: "mp3", speaker: body.speaker || "asteria" }, { returnRawResponse: true });
      const audio = await rawAudioResponse(aura1); if (!audio) return jsonError("TTS providers returned no usable audio."); return audio;
    }
    if (capability === "chat") {
...
async function normalizeCatastrophicSsrResponse(response: Response): Promise<Response> {
  ...
  const body = await response.clone().text(); if (!isH3SwallowedErrorBody(body)) return response; console.error(consumeLastCapturedError() ?? new Error(`h3 swallowed SSR error: ${body}`));
