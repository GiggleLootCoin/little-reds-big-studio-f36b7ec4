...
function asBase64(value: unknown): string | null {
  ...
  if (value && typeof value === "object") for (const key of ["image", "audio", "data", "result", "output", "image_b64"]) {
...
function mediaUrl(value: unknown, keys: string[]): string | null {
  if (typeof value === "string" && /^https?:\/\//i.test(value)) return value;
  if (value && typeof value === "object") for (const key of keys) {
...
function isCapacityError(error: unknown) {
  ...
  return message.includes("3040") || message.toLowerCase().includes("capacity temporarily exceeded") || message.toLowerCase().includes("out of capacity");
}
async function rawAudioResponse(result: unknown): Promise<Response | null> {
  ...
  if (typeof result === "string" && /^https?:\/\//i.test(result)) {
    const upstream = await fetch(result); if (!upstream.ok) return null;
    const headers = new Headers(upstream.headers); headers.set("cache-control", "no-store");
    if (!headers.get("content-type")) headers.set("content-type", "audio/mpeg");
    return new Response(upstream.body, { status: upstream.status, headers });
  }
  if (result && typeof result === "object") for (const key of ["audio", "url", "uri", "result", "output"]) {
    const response = await rawAudioResponse((result as Record<string, unknown>)[key]); if (response) return response;
  }
...
async function rawImageResponse(result: unknown): Promise<Response | null> {
  if (result instanceof Response) {
    if (!result.ok) return result;
    const headers = new Headers(result.headers); headers.set("content-type", headers.get("content-type") || "image/png"); headers.set("cache-control", "no-store");
    return new Response(result.body, { status: result.status, headers });
  }
...
function hasImageContent(messages: unknown[]) {
  return messages.some((message) => Array.isArray((message as { content?: unknown })?.content) && (message as { content: unknown[] }).content.some((part) => (part as { type?: string })?.type === "image_url"));
}
function chatText(result: unknown): string {
  if (typeof result === "string") return result.trim();
  if (result && typeof result === "object") {
    for (const key of ["response", "text", "generated_text", "output", "content"]) {
      const value = (result as Record<string, unknown>)[key]; if (typeof value === "string" && value.trim()) return value.trim();
    }
    ...
    if (Array.isArray(choices)) {
      ...
      if (content && typeof content === "object" && typeof (content as Record<string, unknown>).content === "string") return String((content as Record<string, unknown>).content).trim();
    }
  }
  return "";
}
async function openRouterChat(env: ServerEnv, messages: unknown[]): Promise<unknown> {
  ...
  const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    method: "POST",
    headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json", "HTTP-Referer": "https://little-reds-big-studio-f36b7ec4.gigglelootcoin.workers.dev", "X-Title": "Buddy AI" },
    body: JSON.stringify({ model: "openrouter/free", messages, max_tokens: 1024, temperature: 0.6 }),
  });
  const payload: unknown = await response.json().catch(() => null);
  if (!response.ok) {
    const payloadRecord = payload && typeof payload === "object" ? (payload as Record<string, unknown>) : null;
    const detail = payloadRecord?.error ? JSON.stringify(payloadRecord.error) : `HTTP ${response.status}`;
  ...
  try {
    if (capability === "speech-to-text") {
      ...
      try { result = await env.AI.run("@cf/openai/whisper-large-v3-turbo", input); }
        ...
        catch (secondaryError) { console.warn("Standard Whisper failed", secondaryError); return jsonError(`Speech recognition temporarily unavailable. ${isCapacityError(primaryError) || isCapacityError(secondaryError) ? "The AI service is at capacity." : "The audio request was rejected."}`, 503); }
      }
      const text = chatText(result); if (!text) return jsonError("Whisper returned no usable transcription.", 502);
    ...
    if (capability === "tts") {
      const lang = ttsLanguage(body.language); const requested = String(body.speaker || "").trim(); const auraSpeaker = BUDDY_TO_AURA[requested] || requested.toLowerCase(); let lastError: unknown;
      if (lang === "en" && AURA_EN_SPEAKERS.has(auraSpeaker)) {
        try { const aura = await env.AI.run("@cf/deepgram/aura-2-en", { text: prompt, encoding: "mp3", speaker: auraSpeaker }); const audio = await rawAudioResponse(aura); if (audio) return audio; lastError = new Error("Aura-2 English returned no audio"); }
        catch (error) { lastError = error; console.warn("Aura-2 English failed", error); }
      }
      if (lang === "es" && AURA_ES_SPEAKERS.has(auraSpeaker)) {
        try { const aura = await env.AI.run("@cf/deepgram/aura-2-es", { text: prompt, encoding: "mp3", speaker: auraSpeaker }); const audio = await rawAudioResponse(aura); if (audio) return audio; lastError = new Error("Aura-2 Spanish returned no audio"); }
        catch (error) { lastError = error; console.warn("Aura-2 Spanish failed", error); }
      }
      if ((lang === "en" && AURA_EN_SPEAKERS.has(auraSpeaker)) || (lang === "es" && AURA_ES_SPEAKERS.has(auraSpeaker))) return jsonError(`The selected voice (${requested || auraSpeaker}) could not produce audio.${isCapacityError(lastError) ? " The AI service is at capacity." : ""}`, 503);
      try { const result = await env.AI.run("@cf/myshell-ai/melotts", { prompt, lang }); const audio = await rawAudioResponse(result); if (audio) return audio; lastError = new Error("MeloTTS returned no audio"); }
    ...
    if (capability === "chat") {
      ...
      try { result = await env.AI.run(model, { messages, max_tokens: 1024, temperature: 0.6 }); }
        ...
        try { result = await openRouterChat(env, messages); }
        catch (openRouterError) { console.warn("OpenRouter fallback failed; using GPT-OSS fallback", openRouterError); result = await env.AI.run("@cf/openai/gpt-oss-20b", { messages, max_tokens: 1024, temperature: 0.6 }); }
      }
      const text = chatText(result); if (!text) return jsonError("Buddy model returned no usable response.", 502);
    ...
    if (capability === "music") {
      ...
      const audio = mediaUrl(result, ["audio", "url", "uri", "result", "output"]); if (!audio) return jsonError("Music model returned no usable audio URL.", 502);
    ...
    if (capability === "video") {
      ...
      const video = mediaUrl(result, ["video", "url", "uri", "result", "output"]); if (!video) return jsonError("Video model returned no usable video URL.", 502);
...
async function normalizeCatastrophicSsrResponse(response: Response): Promise<Response> {
  ...
  const body = await response.clone().text(); if (!isH3SwallowedErrorBody(body)) return response;
...
export default {
  async fetch(request: Request, env: unknown, ctx: unknown) {
    try {
      ...
      const handler = await getServerEntry(); const response = await handler.fetch(request, env, ctx); return await normalizeCatastrophicSsrResponse(response);
    } catch (error) {
      console.error(error); return new Response(renderErrorPage(), { status: 500, headers: { "content-type": "text/html; charset=utf-8" } });
    }
  },
};