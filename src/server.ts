...
async function cloudflareAI(request: Request, env: ServerEnv): Promise<Response | null> {
  ...
  const pathCapability = url.pathname.slice(AI_PREFIX.length).split("/")[0].trim(); const capability = String(body.capability || pathCapability).trim(); const prompt = String(body.prompt ?? body.text ?? body.lyrics ?? "").trim();
  ...
  try {
    if (capability === "speech-to-text") {
      ...
      const input = { audio, task: "transcribe", ...(body.language && body.language !== "Auto" ? { language: body.language } : {}), vad_filter: true }; let result: unknown; let lastError: unknown;
      for (let attempt = 0; attempt < 3 && !result; attempt += 1) { try { result = await env.AI.run("@cf/openai/whisper-large-v3-turbo", input); } catch (error) { lastError = error; if (attempt < 2) await new Promise((resolve) => setTimeout(resolve, 250 * 2 ** attempt)); } }
      if (!result) { try { result = await env.AI.run("@cf/openai/whisper", input); } catch (primaryError) { lastError = primaryError || lastError; try { result = await env.AI.run("@cf/openai/whisper", audio); } catch (secondaryError) { lastError = secondaryError || lastError; } } }
      if (!result) { console.warn("Buddy speech recognition failed after current and legacy Whisper routes", lastError); return jsonError(`Speech recognition temporarily unavailable. ${isCapacityError(lastError) ? "The AI service is at capacity; please try again in a moment." : "The speech service did not accept the recording."}`, 503); }
    ...
    if (capability === "web-search") {
      if (!body.searchQuery?.trim()) return jsonError("Search query is required.", 400); const query = encodeURIComponent(body.searchQuery.trim()); const response = await fetch(`https://r.jina.ai/http://www.google.com/search?q=${query}`, { headers: { "user-agent": "LittleRedsBigStudio/1.0" } }); if (!response.ok) return jsonError("Web search temporarily unavailable.", 503); const text = await response.text(); return Response.json({ text }, { headers: { "cache-control": "no-store" } });
    }
    if (capability === "chat") {
      const messages = Array.isArray(body.messages) && body.messages.length ? body.messages : [{ role: "user", content: prompt }]; const result = await openRouterChat(env, messages); const text = chatText(result); if (!text) return jsonError("The AI returned no usable response.", 502); return Response.json({ text }, { headers: { "cache-control": "no-store" } });
    }
    if (capability === "tts") {
      const requestedSpeaker = String(body.speaker || "").trim();
      const normalizedSpeaker = requestedSpeaker.match(/^aura-2-([a-z0-9_]+)-en$/i)?.[1]?.toLowerCase() || requestedSpeaker.toLowerCase();
      const speaker = (AURA_EN_SPEAKERS.has(normalizedSpeaker) ? normalizedSpeaker : BUDDY_TO_AURA[requestedSpeaker]) || "luna";
      const language = ttsLanguage(body.language);
      if (language !== "en") {
        if (language === "es") {
          const spanishSpeaker = AURA_ES_SPEAKERS.has(normalizedSpeaker) ? normalizedSpeaker : "sirio";
          const result = await env.AI.run("@cf/deepgram/aura-2-es", { text: prompt, speaker: spanishSpeaker, encoding: "mp3" });
          const audio = await rawAudioResponse(result); if (!audio) return jsonError("The speech model returned no audio.", 502); return audio;
        }
        return jsonError("Buddy preset voices currently support English and Spanish only.", 400);
      }
      if (!AURA_EN_SPEAKERS.has(speaker)) return jsonError("Selected Buddy preset is not a valid Aura-2 English speaker.", 400);
      const result = await env.AI.run("@cf/deepgram/aura-2-en", { text: prompt, speaker, encoding: "mp3" }); const audio = await rawAudioResponse(result); if (!audio) return jsonError("The speech model returned no audio.", 502); return audio;
    }
