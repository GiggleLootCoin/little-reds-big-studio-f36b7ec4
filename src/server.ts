...
function asBase64(value: unknown): string | null {
  ...
  if (value && typeof value === "object") for (const key of ["image", "audio", "data", "result", "output"]) { const found = asBase64((value as Record<string, unknown>)[key]); if (found) return found; }
  return null;
}
...
async function rawAudioResponse(result: unknown): Promise<Response | null> {
  ...
  return new Response(Uint8Array.from(atob(base64), (c) => c.charCodeAt(0)), { headers: { "content-type": "audio/mpeg", "cache-control": "no-store" } });
}
...
async function cloudflareAI(request: Request, env: ServerEnv): Promise<Response | null> {
  ...
  try {
    if (capability === "image") {
      const result = await env.AI.run("@cf/black-forest-labs/flux-2-klein-9b", { prompt, width: 1024, height: 768 });
      const base64 = asBase64(result); if (!base64) return jsonError("Image model returned no usable image.");
      return new Response(Uint8Array.from(atob(base64), (c) => c.charCodeAt(0)), { headers: { "content-type": "image/png", "cache-control": "no-store" } });
    }
    if (capability === "tts") {
      try {
      ...
      } catch (error) { if (!isCapacityError(error)) console.warn("MeloTTS failed; trying Aura-1", error); }
      const aura = await env.AI.run("@cf/deepgram/aura-1", { text: prompt, encoding: "mp3", speaker: body.speaker || "asteria" }, { returnRawResponse: true });
      const audio = await rawAudioResponse(aura); if (!audio) return jsonError("TTS providers returned no usable audio."); return audio;
    }
    if (capability === "chat") {
    ...
    console.error("Workers AI generation failed", error); return jsonError(error instanceof Error ? error.message : "Workers AI generation failed.");
  }
}
async function normalizeCatastrophicSsrResponse(response: Response): Promise<Response> {
  ...
  console.error(consumeLastCapturedError() ?? new Error(`h3 swallowed SSR error: ${body}`)); return new Response(renderErrorPage(), { status: 500, headers: { "content-type": "text/html; charset=utf-8" } });
}
