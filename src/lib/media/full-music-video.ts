...
const VIDEO_ENDPOINT = "/generate_video";
...
function resultUrl(value: unknown): string | null {
  ...
  if (value && typeof value === "object") {
    for (const key of ["url", "video", "output", "result", "path"]) {
...
async function outputBlob(value: unknown): Promise<Blob> {
  if (typeof Blob !== "undefined" && value instanceof Blob) {
    if (!value.size || !value.type.startsWith("video/"))
      throw new Error("The video engine returned an invalid video artifact.");
    return value;
  }
  ...
  if (!response.ok)
    throw new Error(`The generated video could not be downloaded (${response.status}).`);
