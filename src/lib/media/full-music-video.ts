...
import {
  ...
  validateRenderedMusicVideoAvArtifact,
  type MusicVideoChunk,
...
async function validateRenderedBlob(blob: Blob, expectedDurationSeconds: number): Promise<void> {
  ...
  try {
    ...
    const hasVideoStream = video.videoWidth > 0 && video.videoHeight > 0;
    if (
      !validateRenderedMusicVideoArtifact({
        contentType: blob.type,
    ...
    if (!AudioContextCtor) throw new Error("This browser cannot independently decode the rendered audio track.");

    audioContext = new AudioContextCtor();
    const renderedAudioBuffer = await audioContext.decodeAudioData(
      (await blob.arrayBuffer()).slice(0),
    );
    const streams = inspectMusicVideoAvStreams({
      hasVideoStream,
      hasAudioStream: renderedAudioBuffer.numberOfChannels > 0 && renderedAudioBuffer.duration > 0,
      videoDurationSeconds: video.duration,
      audioDurationSeconds: renderedAudioBuffer.duration,
    });

    if (
      !validateRenderedMusicVideoAvArtifact({
        contentType: blob.type,
        durationSeconds: streams.videoDurationSeconds,
        audioDurationSeconds: streams.audioDurationSeconds,
        hasVideoStream: streams.hasVideoStream,
        hasAudioStream: streams.hasAudioStream,
        expectedDurationSeconds,
        byteLength: blob.size,
      })
    ) {
      throw new Error(
        `The rendered music video does not contain a synchronized audio/video pair (video ${streams.videoDurationSeconds.toFixed(2)}s, audio ${streams.audioDurationSeconds.toFixed(2)}s).`,
      );
    }
  } finally {
...
export async function generateFullMusicVideo(
  ...
  const engines = [...MUSIC_VIDEO_ENGINE_SPACES];
  ...
  for (const engine of engines) {
    ...
    try {
      ...
      progress(options.onProgress, {
        ...
        total: plan.chunks.length,
        message: "Full music video rendered successfully with synchronized song audio.",
      });
      return {
        blob: rendered.blob,
