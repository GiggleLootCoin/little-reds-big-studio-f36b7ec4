...
async function connect(space: string) {
  const id = spaceId(space); let cached = clients.get(id);
...
async function getApi(space: string) {
  const id = spaceId(space); const cached = apis.get(id); if (cached && cached.expires > Date.now()) return cached.api;
  const cl = await connect(id); if (!cl.view_api) throw new Error("Voice service schema unavailable");
  const a = await cl.view_api(); if (!Object.keys(endpoints(a)).length) throw new Error("Voice service has no usable endpoints");
  apis.set(id, { api: a, expires: Date.now() + 120000 }); return a;
}
...
async function runCloudflare(provider: FreeRunner, input: StudioJobInput, capability: StudioCapability): Promise<StudioArtifact> {
  ...
  const payload: Record<string, unknown> = { capability, prompt, language: input.language, messages: input.messages };
...
async function prepareVoice(capability: StudioCapability, input: StudioJobInput) {
  ...
  const next = { ...input }; const language = localStorage.getItem("buddy-language"); const choice = localStorage.getItem("buddy-voice-choice");
  if (language && language !== "Auto") next.language = language;
  if (choice === "My voice") { const sample = await getVoiceSample(); if (sample) return { capability: "voice-clone" as StudioCapability, input: { ...next, audio: sample, refAudio: sample, target_text: input.target_text ?? input.text ?? input.prompt ?? "" } }; }
...
export function artifactText(value: unknown): string {
  ...
  if (value && typeof value === "object") for (const key of ["text", "response", "generated_text", "transcription", "transcript", "content", "value", "data", "output", "result"]) { const text = artifactText((value as Record<string, unknown>)[key]); if (text) return text; }
  return "";
}
