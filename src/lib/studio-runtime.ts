...
async function runProductionRedClone(sample: Blob, refText: string, text: string, language: string, onStatus?: (s: string) => void) {
  onStatus?.("Generating Buddy's Red voice…");
  const referenceId = await redReferenceId(sample);
  const alreadyEncoded = cachedRedReferenceId === referenceId && cachedRedReferenceBase64.length > 0;
  if (!alreadyEncoded) {
  ...
  if (normalized.stats.duration <= 0 || normalized.stats.peak <= 0 || normalized.stats.rms <= 0) throw new Error("Red voice generation returned silent or unusable audio.");
  return { url: normalized.url, provider: response.headers.get("x-clone-provider") || "Production Red reference clone", verification: "Production Red reference clone + browser audio decode + non-silent verification", duration: normalized.stats.duration, peak: normalized.stats.peak, rms: normalized.stats.rms };
}
async function runVerifiedClone(sample: Blob, refText: string, text: string, language: string, onStatus?: (s: string) => void, modelSize: "0.6B" | "1.7B" = "1.7B", persist = true) {
  ...
  try {
    ...
    try {
      ...
      const fallback = await runtime.runStudioJob("voice-clone", { refAudio: sample, referenceAudio: sample, audio: sample, refText, referenceTranscript: refText, target_text: text, text, language, _skipProviders: ["hf-qwen3-tts"] }, onStatus);
      if (!fallback.url) throw primaryError;
      const fallbackBlob = await fetch(fallback.url).then(response => { if (!response.ok) throw new Error(`Fallback audio download failed (${response.status}).`); return response.blob(); });
      const normalized = await normalizeAndVerifyBrowserAudio(fallbackBlob);
      if (normalized.stats.duration <= 0 || normalized.stats.peak <= 0 || normalized.stats.rms <= 0) throw new Error("Fallback clone returned silent or unusable audio.");
      if (persist) await saveBuddyClonePreview(normalized.blob, fallback.provider);
      result = { url: normalized.url, provider: fallback.provider, verification: `Free fallback ${fallback.provider} + browser audio decode + non-silent artifact verification`, duration: normalized.stats.duration, peak: normalized.stats.peak, rms: normalized.stats.rms };
    } catch (fallbackError) {
      throw new Error(`Voice generation failed on the primary and free fallback. ${fallbackError instanceof Error ? fallbackError.message : String(primaryError)}`);
    }
  }
...
async function prepareSpeechToText(input: StudioJobInput): Promise<StudioJobInput> {
  ...
  try { return { ...input, audio: (await normalizeAndVerifyBrowserAudio(audio)).blob }; }
  catch (error) { throw new Error(`The microphone recording could not be decoded. ${error instanceof Error ? error.message : String(error)}`); }
}
