...
async function runVerifiedClone(sample: Blob, refText: string, text: string, language: string, onStatus?: (s: string) => void, modelSize: "0.6B" | "1.7B" = "1.7B") {
  let result;
  try {
    ...
    try {
      ...
      throw new Error(`Voice generation failed on Qwen and the free fallback. ${fallbackError instanceof Error ? fallbackError.message : String(primaryError)}`);
    }
  }
...
export async function runStudioJob(capability: StudioCapability, input: StudioJobInput, onStatus?: (s: string) => void): Promise<StudioArtifact> {
  if (capability === "voice-clone") {
    ...
    const result = await runVerifiedClone(sample, refText, targetText, language, onStatus, modelSize);
