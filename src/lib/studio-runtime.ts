...
export type StudioCapability =
  | "chat" | "speech-to-text" | "music" | "image" | "video"
  | "voice-clone" | "voice-swap" | "vocal-separation" | "tts";
...
function artifactUrl(v: unknown): string | null {
  ...
  if (v && typeof v === "object") for (const key of ["url", "uri", "src", "path", "value", "data"]) { const u = artifactUrl((v as Record<string, unknown>)[key]); if (u) return u; }
  return null;
}
