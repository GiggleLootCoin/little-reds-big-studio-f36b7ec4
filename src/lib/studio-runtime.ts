...
function validateArtifact(capability: StudioCapability, value: unknown, url: string | null) {
  ...
  if (["music", "image", "video", "voice-clone", "voice-swap", "vocal-separation", "tts"].includes(capability) && !url) {
...
async function runOn(provider: FreeRunner, input: StudioJobInput, capability: StudioCapability) {
  ...
  const args = await Promise.all(build(candidates[0].ep, input).map(async (v) => typeof Blob !== "undefined" && v instanceof Blob ? handle_file(v) : v));
