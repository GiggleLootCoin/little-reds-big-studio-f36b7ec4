...
const VERSION = "voice-clone-v10.1-turbo-cached-reference";
...
type CloneBody = { referenceId?: string; audioBase64?: string; audio?: string; refAudio?: string; referenceAudio?: string; audioMimeType?: string; text?: string; target_text?: string; prompt?: string };
type Input = { bytes: Uint8Array; mime: string; name: string; text: string };
type CachedReference = { path: string; expires: number; bytes: number };
...
async function readInput(request: Request): Promise<{ input: Input | null; referenceId: string; text: string }> {
  if ((request.headers.get("content-type") || "").toLowerCase().includes("multipart/form-data")) {
    ...
    if (!file?.size) return { input: null, referenceId, text };
    return { input: { bytes: new Uint8Array(await file.arrayBuffer()), mime: normalizeMime(file.type, file.name), name: file.name || "voice-reference.wav", text }, referenceId, text };
  }
  let body: CloneBody;
  ...
  const text = String(body.text || body.target_text || body.prompt || "").trim();
  if (!encoded) return { input: null, referenceId: String(body.referenceId || "").trim(), text };
  const d = decodeBase64(encoded, body.audioMimeType || "audio/wav");
  return { input: { bytes: d.bytes, mime: d.mime, name: "voice-reference.wav", text }, referenceId: String(body.referenceId || "").trim(), text };
}
...
async function referencePath(input: Input, referenceSha256: string, env: CloneEnv) {
  ...
  if (!input) throw new Error("Cached voice reference is unavailable.");
  const path = await uploadReference(input, env);
  const cached = { path, expires: now + REFERENCE_CACHE_TTL_MS, bytes: input.bytes.byteLength };
  referenceCache.set(referenceSha256, cached);
  return cached;
}
...
async function generate(space: string, path: string, text: string, env: CloneEnv) {
  ...
  const q = await fetch(`${space}/gradio_api/call/generate`, { method: "POST", headers: { ...(env.HF_TOKEN ? { Authorization: `Bearer ${env.HF_TOKEN}` } : {}), "content-type": "application/json" }, body: JSON.stringify({ data: [text.slice(0, 300), file, 0.8, 0, 0, 0.95, 1000, 1.2, true] }) });
  ...
  for (const block of stream.split(/\n\s*\n/)) { const ev = block.match(/^event:\s*(.+)$/m)?.[1]?.trim(); const data = block.match(/^data:\s*(.*)$/m)?.[1]?.trim() || ""; if (ev === "complete") complete = data; if (ev === "error") failure = data; }
  ...
  let output: unknown; try { output = JSON.parse(complete); } catch { throw new Error("Chatterbox Turbo returned malformed completion data."); }
...
export async function handleProductionVoiceClone(request: Request, env: CloneEnv) {
  ...
  try {
    ...
    let referenceSha256 = parsed.referenceId;
    if (input) {
      if (input.bytes.length < 4096) throw new Error("The voice sample is too short or empty.");
      const computed = await sha256(input.bytes);
      referenceSha256 = computed || referenceSha256;
    }
    if (!referenceSha256) throw new Error("A voice reference ID is required.");
    const text = parsed.text || DEFAULT_TEXT;
    if (env.CHATTERBOX_ENDPOINT && input) {
      try { const r = await cloneViaSelfHosted(input, text, env, referenceSha256); if (r) return r; } catch (e) { console.warn(`[voice-clone] self-hosted failed: ${e instanceof Error ? e.message : String(e)}`); }
    }
    let cached = referenceCache.get(referenceSha256);
    if (!cached || cached.expires <= Date.now()) {
      if (!input) return errorJson("Voice reference cache expired; refresh required.", 428);
      cached = await referencePath(input, referenceSha256, env);
    }
    try {
      const bytes = await generate(HF_TURBO_SPACE, cached.path, text, env);
      return await audioResponse(bytes, "Chatterbox Turbo — Resemble AI Hugging Face ZeroGPU", referenceSha256, cached.bytes);
    } catch (firstError) {
      referenceCache.delete(referenceSha256);
      if (!input) return errorJson("Voice reference expired; refresh required.", 428);
      cached = await referencePath(input, referenceSha256, env);
      try {
        const bytes = await generate(HF_TURBO_SPACE, cached.path, text, env);
        return await audioResponse(bytes, "Chatterbox Turbo — Resemble AI Hugging Face ZeroGPU (reference refreshed)", referenceSha256, cached.bytes);
      } catch (secondError) {
