...
const VERSION = "voice-clone-v7.0";
...
function audioResponse(
  bytes: Uint8Array,
  contentType: string,
  provider = "Chatterbox via Hugging Face Inference Providers",
): Response {
...
async function cloneViaHuggingFaceSpace(
  ...
  const upload = await fetch(`${SPACE}/gradio_api/upload`, {
    method: "POST",
  ...
  if (!upload.ok) {
    ...
    throw new Error(
      `Hugging Face Chatterbox Space upload failed (${upload.status})${detail ? `: ${detail}` : ""}`,
    );
  }
  const uploaded = (await upload.json()) as unknown;
  const uploadedPath = Array.isArray(uploaded) ? String(uploaded[0] || "") : "";
  if (!uploadedPath)
    throw new Error("Hugging Face Chatterbox Space did not return an uploaded file path.");
  const call = await fetch(`${SPACE}/gradio_api/call/v2/generate_tts_audio`, {
    method: "POST",
    ...
    body: JSON.stringify({
      data: [
        text.slice(0, 300),
        {
          ...
          orig_name: input.name || "voice-reference.wav",
        },
        0.5,
        0.8,
        0,
  ...
  if (!call.ok) {
    const detail = (await call.text()).slice(0, 1000);
    throw new Error(
      `Hugging Face Chatterbox Space queue failed (${call.status})${detail ? `: ${detail}` : ""}`,
    );
  }
  const callPayload = (await call.json()) as { event_id?: string };
  if (!callPayload.event_id)
    throw new Error("Hugging Face Chatterbox Space did not return a queue event.");
  const result = await fetch(
    `${SPACE}/gradio_api/call/generate_tts_audio/${encodeURIComponent(callPayload.event_id)}`,
    { headers: { Authorization: `Bearer ${token}` } },
  );
  if (!result.ok) {
    const detail = (await result.text()).slice(0, 1200);
    throw new Error(
      `Hugging Face Chatterbox Space generation failed (${result.status})${detail ? `: ${detail}` : ""}`,
    );
  }
  const stream = await result.text();
  const completeLine = stream
    .split("\n")
    .reverse()
    .find((line) => line.startsWith("data:") && line.includes("["));
  if (!completeLine)
    throw new Error("Hugging Face Chatterbox Space returned no completed audio event.");
  let outputs: unknown;
  try {
    outputs = JSON.parse(completeLine.slice(5).trim());
  } catch {
    throw new Error("Hugging Face Chatterbox Space returned malformed completion data.");
  }
  ...
  const outputUrl =
    typeof first === "string"
      ...
      : first && typeof first === "object"
        ? String(
            (first as Record<string, unknown>).url || (first as Record<string, unknown>).path || "",
          )
        : "";
  if (!outputUrl)
    throw new Error("Hugging Face Chatterbox Space completed without an audio file URL.");
  const audioUrl = outputUrl.startsWith("http")
    ? outputUrl
    : `${SPACE}/gradio_api/file=${outputUrl}`;
  const audio = await fetch(audioUrl, { headers: { Authorization: `Bearer ${token}` } });
  if (!audio.ok)
    throw new Error(`Hugging Face Chatterbox Space audio fetch failed (${audio.status}).`);
  return audioResponse(
    new Uint8Array(await audio.arrayBuffer()),
    audio.headers.get("content-type") || "audio/wav",
    "Chatterbox via Hugging Face Space (ZeroGPU fallback)",
  );
}
  ...
  return Response.json(
    {
      ...
      uploadField: "audio",
      fallback:
        "ResembleAI/Chatterbox Hugging Face Space when Inference Provider credits are unavailable",
    },
    { headers: headers() },
  );
}
  ...
  try {
    ...
    if (!upstream.ok) {
      if ([402, 429, 503].includes(upstream.status)) {
        try {
          return await cloneViaHuggingFaceSpace(input, text, env.HF_TOKEN);
        } catch (fallbackError) {
          const detail = new TextDecoder().decode(raw).slice(0, 800);
          const fallbackMessage =
            fallbackError instanceof Error
              ? fallbackError.message
              : "Hugging Face Space fallback failed.";
          throw new Error(
            `Chatterbox Inference Provider unavailable (${upstream.status}); Space fallback also failed: ${fallbackMessage}${detail ? ` Provider detail: ${detail}` : ""}`,
          );
        }
      }
