...
const DEFAULT_TEXT = "Hi. I'm Buddy. This is my new voice. Let's make something brilliant together.";
...
export function voiceCloneHealth(): Response {
  return Response.json(
    { ok: true, capability: "voice-clone", backend: BACKEND, version: VERSION, transcriptRequired: false },
    { headers: headers() },
  );
}

export async function handleProductionVoiceClone(request: Request, env: CloneEnv): Promise<Response> {
  if (request.method !== "POST") return Response.json({ ok: false, error: "POST required." }, { status: 405, headers: headers() });
  if (!env.HF_TOKEN) return Response.json({ ok: false, backend: BACKEND, version: VERSION, error: "Hugging Face voice service is not configured." }, { status: 503, headers: headers() });

  let body: CloneBody;
  try {
    body = (await request.json()) as CloneBody;
  } catch {
    return Response.json({ ok: false, error: "The clone request was not valid JSON." }, { status: 400, headers: headers() });
  }

  const encoded = body.audioBase64 || body.audio || body.refAudio || body.referenceAudio;
  if (!encoded) return Response.json({ ok: false, error: "A voice sample is required." }, { status: 400, headers: headers() });

  try {
    const decoded = decodeAudio(encoded);
    if (decoded.bytes.byteLength < 4096) throw new Error("The voice sample is too short or empty.");
    const text = body.text?.trim() || body.target_text?.trim() || body.prompt?.trim() || DEFAULT_TEXT;
    const upstream = await fetch(ROUTE, {
      ...
      body: JSON.stringify({ text: text.slice(0, 5000), audio_url: toDataUri(decoded.bytes, body.audioMimeType || decoded.mime), exaggeration: 0.5, temperature: 0.8, cfg: 0.5, seed: 0 }),
    });
    ...
    if (type.toLowerCase().includes("json")) {
      ...
      if (payload.audio?.file_data) return audioResponse(decodeAudio(payload.audio.file_data).bytes, payload.audio.content_type || "audio/wav");
    ...
    return Response.json(
      { ok: false, backend: BACKEND, version: VERSION, error: error instanceof Error ? error.message : "Voice cloning failed." },
