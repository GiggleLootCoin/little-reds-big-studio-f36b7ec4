...
function numericSamples(v: unknown): number[] | null {
  if (v instanceof Float32Array || v instanceof Float64Array || v instanceof Int16Array) return Array.from(v);
  if (Array.isArray(v) && v.length && v.every((x) => typeof x === "number" && Number.isFinite(x))) return v as number[];
  return null;
}
...
function artifactUrl(v: unknown): string | null {
  ...
  if (v && typeof v === "object") {
    ...
    for (const k of ["url", "uri", "src", "path", "value", "audio", "file", "data"]) {
      const u = artifactUrl(o[k]); if (u) return u;
    }
  }
  return null;
}
function pick(name: string, input: StudioJobInput) {
  ...
  const aliases: Record<string, string[]> = {
    prompt: ["text", "message", "query", "lyrics"], text: ["prompt", "message", "targettext", "lyrics"],
    audio: ["inputaudio", "sourceaudio", "referenceaudio", "refaudio", "file"],
    referenceaudio: ["refaudio", "audio", "file"], refaudio: ["referenceaudio", "audio", "file"],
    targettext: ["text", "prompt"], language: ["lang"], speaker: ["voice", "speakername"],
  };
...
function buildArgs(ep: Endpoint, input: StudioJobInput, capability: StudioCapability) {
  return (ep.parameters ?? []).map((p) => {
    ...
    if (capability === "voice-clone" && /refaudio|referenceaudio|referencevoice|voiceprompt/.test(name)) throw new Error("Clone endpoint requires the saved reference voice");
    const f = fallback(p);
...
async function connect(space: string) {
  ...
  const existing = clients.get(id); if (existing) return existing;
  const sources = typeof window === "undefined" ? [id] : [proxyOrigin(id), id];
  const promise = (async () => {
    let last: unknown;
    for (const source of sources) for (let attempt = 0; attempt < 3; attempt++) {
      try { return await Client.connect(source, { status_callback: () => undefined }); }
  ...
  promise.catch(() => clients.delete(id)); clients.set(id, promise); return promise;
}
async function getApi(space: string) {
  const id = spaceId(space); const cached = apis.get(id);
  if (cached && cached.expires > Date.now()) return cached.api;
  const cl = await connect(id); if (!cl.view_api) throw new Error("Voice service schema unavailable");
  const api = await cl.view_api(); if (!Object.keys(endpoints(api)).length) throw new Error("Voice service has no callable endpoints");
  apis.set(id, { api, expires: Date.now() + 120000 }); return api;
}
function score(name: string, ep: Endpoint, capability: StudioCapability, input: StudioJobInput) {
  const h = norm(`${name} ${ep.fn ?? ""} ${ep.description ?? ""}`); let s = 0;
  const words = capability === "voice-clone" ? ["clone", "zero", "reference", "voice"] : capability === "tts" ? ["tts", "speech", "voice", "generate"] : [capability.replace(/-/g, ""), "generate", "create", "text", "audio", "image", "video", "music"];
  for (const w of words) if (h.includes(w)) s += 6;
  if (capability === "voice-clone" && (h.includes("clone") || h.includes("reference"))) s += 30;
  for (const p of ep.parameters ?? []) if (pick(p.parameter_name ?? p.label ?? "", input) !== undefined) s += 3;
  return s;
}
async function runCloudflare(provider: FreeRunner, input: StudioJobInput, capability: StudioCapability): Promise<StudioArtifact> {
  ...
  const payload: Record<string, unknown> = { capability, prompt, text: prompt, language: input.language, speaker: input.speaker, messages: input.messages };
  if (input.audio instanceof Blob) payload.audio = input.audio;
  const response = await fetch(provider.url, { method: "POST", body: JSON.stringify(payload), headers: { "content-type": "application/json" } });
  ...
  const value = await response.json(); if (!output(value)) throw new Error(`${provider.name}: empty result`);
...
async function runQwen(cl: Client, input: StudioJobInput, capability: StudioCapability): Promise<StudioArtifact> {
  if (capability === "tts") {
    const text = String(input.text ?? input.target_text ?? input.prompt ?? "").trim(); if (!text) throw new Error("Voice text is empty");
    const speaker = String(input.speaker ?? "Ryan"); const language = String(input.language ?? "English");
    let last: unknown;
    for (const size of [String(input.model_size ?? "1.7B"), "0.6B"]) try {
      const r = await cl.predict("/generate_custom_voice", [text, language, speaker, String(input.instruction ?? ""), size]);
      const value = Array.isArray(r) ? r : (r as { data?: unknown[] }).data ?? r; const url = artifactUrl(value);
      if (!url) throw new Error("Qwen returned no playable audio"); return { capability, value, url, provider: `Qwen3-TTS (${speaker})` };
    } catch (e) { last = e; }
  ...
  const text = String(input.target_text ?? input.text ?? input.prompt ?? "").trim(); if (!text) throw new Error("Voice clone text is empty");
  const ref = await handle_file(sample); const refText = String(input.referenceTranscript ?? input.refText ?? ""); let last: unknown;
  for (const size of [String(input.model_size ?? "0.6B"), "1.7B"]) try {
    const r = await cl.predict("/generate_voice_clone", [ref, refText, text, String(input.language ?? "English"), !refText, size]);
    const value = Array.isArray(r) ? r : (r as { data?: unknown[] }).data ?? r; const url = artifactUrl(value);
    if (!url) throw new Error("Qwen returned no playable cloned audio"); return { capability, value, url, provider: "Qwen3-TTS Voice Clone" };
  } catch (e) { last = e; }
...
async function runSpace(provider: FreeRunner, input: StudioJobInput, capability: StudioCapability): Promise<StudioArtifact> {
  const space = spaceId(provider.url); const cl = await connect(space);
  if (space === "Qwen/Qwen3-TTS" && (capability === "tts" || capability === "voice-clone")) return runQwen(cl, input, capability);
  const api = endpoints(await getApi(space)); const candidates = Object.entries(api).map(([name, ep]) => ({ name, ep, score: score(name, ep, capability, input) })).sort((a, b) => b.score - a.score);
  ...
  for (const c of candidates) {
    try {
      ...
      const r = await cl.predict(c.name, args); const value = Array.isArray(r) ? r : (r as { data?: unknown[] }).data ?? r; const url = artifactUrl(value);
      if (!output(value)) throw new Error("Provider returned no usable result");
      if (["tts", "voice-clone", "voice-swap", "music", "image", "video", "vocal-separation"].includes(capability) && !url) throw new Error("Provider returned no playable media");
...
async function prepareVoice(capability: StudioCapability, input: StudioJobInput) {
  ...
  const profile = getBuddyVoiceProfile(); const next = { ...input };
  if (profile.language && profile.language !== "Auto") next.language = profile.language;
  if (profile.mode === "clone") {
    const sample = await getBuddyVoiceSample(); if (!sample) throw new Error("Your saved Buddy voice sample is unavailable. Record it again.");
    next.audio = sample; next.refAudio = sample; next.referenceAudio = sample; next.target_text = input.target_text ?? input.text ?? input.prompt ?? "";
    next.referenceTranscript = profile.referenceTranscript ?? ""; next.refText = profile.referenceTranscript ?? "";
    return { capability: "voice-clone" as StudioCapability, input: next };
  }
  next.speaker = profile.speaker; next.text = input.text ?? input.target_text ?? input.prompt ?? "";
...
export async function runStudioJob(capability: StudioCapability, input: StudioJobInput, onStatus?: (s: string) => void): Promise<StudioArtifact> {
  ...
  for (const provider of providers) {
    try {
      onStatus?.("Working…");
      const result = await Promise.race([provider.url.startsWith("/api/ai/") ? runCloudflare(provider, prepared.input, prepared.capability) : runSpace(provider, prepared.input, prepared.capability), new Promise<never>((_, reject) => window.setTimeout(() => reject(new Error("Timed out")), 180000))]);
      if (!result.url && ["tts", "voice-clone", "voice-swap", "music", "image", "video", "vocal-separation"].includes(prepared.capability)) throw new Error("No playable artifact returned");
      onStatus?.("Ready."); return { ...result, capability };
    } catch (e) { failures.push(`${provider.name}: ${e instanceof Error ? e.message : String(e)}`); }
  }
