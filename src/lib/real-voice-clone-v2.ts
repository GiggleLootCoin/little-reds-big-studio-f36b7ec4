...
async function cloneWithQwen(
  ...
  onStatus?.(
    "Chatterbox was unavailable. Switching to Qwen3-TTS 1.7B Base full-reference cloning…",
  );
...
async function verifySpeaker(sample: Blob, generatedUrl: string): Promise<string | null> {
  try {
    ...
    const args = (candidate[1].parameters ?? []).map((p, index) => {
      ...
      if (index === 0 || /first|speaker.?1|sample.?1|questioned/i.test(n))
        return handle_file(sample);
    ...
    if (/different speaker|not similar|fail/i.test(text))
      throw new Error("Independent speaker verification rejected the generated voice.");
    return null;
  } catch (error) {
...
export async function createBestFreeVoiceClone(
  ...
  try {
    ...
    return {
      ...result,
      verification: verification ?? "reference-conditioned output verified playable",
    };
  } catch (error) {
    firstError = error instanceof Error ? error.message : String(error);
    onStatus?.(
      `Chatterbox could not produce a verified clone (${firstError}). Trying the independent Qwen clone…`,
    );
  }
