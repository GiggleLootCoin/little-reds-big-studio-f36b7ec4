...
async function cloneWithChatterbox(sample: Blob, text: string, onStatus?: (s: string) => void): Promise<CloneResult> {
  onStatus?.("Qwen was unavailable. Trying Chatterbox Multilingual with your actual reference audio…");
  const client = await Client.connect(CHATTERBOX);
  const reference = await handle_file(sample);
  const result = await client.predict("/generate_tts_audio", [text.slice(0, 300), "en", reference, 0.5, 0.8, 0, 0.5]);
  const url = audioUrl(resultData(result));
...
async function verifySpeaker(sample: Blob, generatedUrl: string): Promise<string> {
  ...
  if (!response.ok) throw new Error("Generated clone audio could not be read back for verification.");
  ...
  if (!candidate) throw new Error("Independent speaker verification is unavailable; the clone cannot be marked verified.");
  ...
  if (percent) {
    ...
    if (score < 70) throw new Error(`Speaker verification rejected the clone (${score}% similarity).`);
  ...
  throw new Error("Independent speaker verification did not confirm that the generated audio matches the uploaded voice.");
}
...
export async function createBestFreeVoiceClone(sample: Blob, refText: string, text: string, onStatus?: (s: string) => void): Promise<CloneResult> {
  ...
  try {
    ...
    throw new Error(`No genuine voice clone was verified. Qwen: ${qwenError}. Chatterbox: ${reason}`);
  }
}
