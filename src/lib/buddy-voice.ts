...
const DB_NAME = "little-reds-big-studio",
  STORE = "voice-profile",
  SAMPLE_KEY = "buddy-voice-sample",
  CLONE_PREVIEW_KEY = "buddy-voice-clone-preview";
...
async function openVoiceDb(): Promise<IDBDatabase> {
  ...
  }), STORAGE_TIMEOUT_MS, "[voice-storage] Opening local voice storage timed out.");
}
...
async function normalizeReferenceAudio(blob: Blob): Promise<Blob> {
  ...
  try {
    ...
    for (let channel = 0; channel < channels; channel++) { const source = buffer.getChannelData(channel); for (let i = 0; i < buffer.length; i++) samples[i] += source[i] / channels; }
  ...
  } catch (error) { if (error instanceof Error && error.message.startsWith("[audio-decode]")) throw error; return blob; } finally { await context.close().catch(() => undefined); }
}

async function putVoiceValue(key: string, value: unknown): Promise<void> {
  const db = await openVoiceDb();
  try { await withTimeout(new Promise<void>((resolve, reject) => { const tx = db.transaction(STORE, "readwrite"); tx.objectStore(STORE).put(value, key); tx.oncomplete = () => resolve(); tx.onerror = () => reject(tx.error || new Error("Could not save voice state.")); }), STORAGE_TIMEOUT_MS, "[voice-storage] Saving voice state timed out."); } finally { db.close(); }
}
async function getVoiceValue<T>(key: string): Promise<T | null> {
  const db = await openVoiceDb();
  try { return await withTimeout(new Promise<T | null>((resolve, reject) => { const req = db.transaction(STORE, "readonly").objectStore(STORE).get(key); req.onsuccess = () => resolve((req.result as T) ?? null); req.onerror = () => reject(req.error || new Error("Could not read voice state.")); }), STORAGE_TIMEOUT_MS, "[voice-storage] Reading local voice state timed out."); } catch { return null; } finally { db.close(); }
}
...
function legacyProfile(): Partial<BuddyVoiceProfile> | null {
  ...
  for (const key of ["buddy-voice-choice", "buddyVoiceChoice"]) { try { const raw = localStorage.getItem(key); if (!raw) continue; const value = JSON.parse(raw) as unknown; if (typeof value === "string") return { mode: "preset", speaker: value }; if (value && typeof value === "object") { const item = value as Record<string, unknown>; return { mode: item.mode === "clone" ? "clone" : "preset", speaker: typeof item.speaker === "string" ? item.speaker : undefined, language: typeof item.language === "string" ? item.language : undefined, mood: typeof item.mood === "string" ? item.mood : undefined, tone: typeof item.tone === "string" ? item.tone : undefined, referenceTranscript: typeof item.referenceTranscript === "string" ? item.referenceTranscript : undefined }; } } catch {} }
  return null;
}
