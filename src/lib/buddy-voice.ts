...
function isLegacyRyanDefault(profile: Partial<BuddyVoiceProfile> | null) {
  return Boolean(
    profile && profile.mode === "preset" && (!profile.speaker || profile.speaker === "Ryan"),
  );
}
export function getBuddyVoiceProfile(): BuddyVoiceProfile {
  ...
  try {
    ...
    const migrated =
      localStorage.getItem(RED_DEFAULT_MIGRATION_KEY) !== "1" &&
      (selected == null || isLegacyRyanDefault(selected));
    if (migrated) {
