...
export const BUDDY_VOICE_PRESETS: readonly BuddyVoicePreset[] = [
  { id: "Ryan", label: "Rook", note: "Low-mid, rhythmic, effortlessly assured", nativeLanguage: "English", languages: ["English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Chinese", "Japanese", "Korean"], character: "Dry wit, street-smart warmth, never in a rush." },
  { id: "Aiden", label: "Sunny Vale", note: "Clear, bright, open and naturally upbeat", nativeLanguage: "English", languages: ["English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Chinese", "Japanese", "Korean"], character: "The mate who makes a complicated thing suddenly feel doable." },
  { id: "Vivian", label: "Velvet Circuit", note: "Bright edge with a polished, animated lift", nativeLanguage: "Chinese", languages: ["Chinese", "English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Japanese", "Korean"], character: "Quick-minded, expressive and slightly mischievous." },
  { id: "Serena", label: "Honey Static", note: "Gentle warmth with a close, intimate presence", nativeLanguage: "Chinese", languages: ["Chinese", "English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Japanese", "Korean"], character: "Soft around the edges, observant, reassuring without going syrupy." },
  { id: "Uncle_Fu", label: "Night Train", note: "Deep, mellow and grounded with quiet authority", nativeLanguage: "Chinese", languages: ["Chinese", "English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Japanese", "Korean"], character: "Steady storyteller energy. Calm enough to make chaos sit down." },
  { id: "Dylan", label: "Sidewinder", note: "Youthful, clear and naturally punchy", nativeLanguage: "Chinese (Beijing)", languages: ["Chinese", "English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Japanese", "Korean"], character: "Fast on the comeback, playful without becoming cartoonish." },
  { id: "Eric", label: "Afterglow", note: "Husky brightness with lively forward motion", nativeLanguage: "Chinese (Sichuan)", languages: ["Chinese", "English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Japanese", "Korean"], character: "Big personality, bright eyes, a little cinematic when the moment earns it." },
  { id: "Ono_Anna", label: "Moonwire", note: "Light, nimble and playful with delicate phrasing", nativeLanguage: "Japanese", languages: ["Japanese", "Chinese", "English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Korean"], character: "Curious, clever and quietly sparkling." },
  { id: "Sohee", label: "Emberline", note: "Warm emotional colour with a rich, human centre", nativeLanguage: "Korean", languages: ["Korean", "Chinese", "English", "German", "French", "Spanish", "Italian", "Portuguese", "Russian", "Japanese"], character: "Tender when needed, grounded when it matters, never overly theatrical." },
] as const;
...
const DEFAULT_PROFILE: BuddyVoiceProfile = { mode: "preset", speaker: "Ryan", language: "English", mood: "natural", tone: "conversational" };
const DB_NAME = "little-reds-big-studio", STORE = "voice-profile", SAMPLE_KEY = "buddy-voice-sample", CLONE_PREVIEW_KEY = "buddy-voice-clone-preview";
...
async function normalizeReferenceAudio(blob: Blob): Promise<Blob> {
  ...
  try {
    ...
    for (let i = 0; i < pcm.length; i++) { const sample = Math.max(-1, Math.min(1, samples[i])); pcm[i] = sample < 0 ? sample * 0x8000 : sample * 0x7fff; }
...
async function putVoiceValue(key: string, value: unknown): Promise<void> {
  ...
  try {
    ...
    }), STORAGE_TIMEOUT_MS, "[voice-storage] Saving voice state timed out.");
  } finally { db.close(); }
}
...
async function getVoiceValue<T>(key: string): Promise<T | null> {
  ...
  try {
    ...
    }), STORAGE_TIMEOUT_MS, "[voice-storage] Reading local voice state timed out.");
...
export async function clearBuddyVoiceSample(): Promise<void> { try { const db = await openVoiceDb(); try { await withTimeout(new Promise<void>((resolve, reject) => { const tx = db.transaction(STORE, "readwrite"); tx.objectStore(STORE).delete(SAMPLE_KEY); tx.objectStore(STORE).delete(CLONE_PREVIEW_KEY); tx.oncomplete = () => resolve(); tx.onerror = () => reject(tx.error); }), STORAGE_TIMEOUT_MS, "[voice-storage] Clearing local voice state timed out."); } finally { db.close(); } } catch {} }
  ...
  for (const key of ["buddy-voice-choice", "buddyVoiceChoice"]) {
    try {
      ...
      if (value && typeof value === "object") {
        ...
        return { mode: item.mode === "clone" ? "clone" : "preset", speaker: typeof item.speaker === "string" ? item.speaker : undefined, language: typeof item.language === "string" ? item.language : undefined, mood: typeof item.mood === "string" ? item.mood : undefined, tone: typeof item.tone === "string" ? item.tone : undefined, referenceTranscript: typeof item.referenceTranscript === "string" ? item.referenceTranscript : undefined };
      }
    } catch {}
  }
  return null;
...
export async function clearBuddyVoiceClone() { const profile = getBuddyVoiceProfile(); await clearBuddyVoiceSample(); saveBuddyVoiceProfile({ mode: "preset", speaker: profile.speaker || "Ryan", language: profile.language || "English", mood: profile.mood || "natural", tone: profile.tone || "conversational", cloneVerified: false }); }
