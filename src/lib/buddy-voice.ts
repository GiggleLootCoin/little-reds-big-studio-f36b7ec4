...
async function normalizeReferenceAudio(blob: Blob): Promise<Blob> {
  ...
  try {
    ...
    view.setUint32(28, buffer.sampleRate * 2, true); view.setUint16(32, 2, true); view.setUint16(34, 16, true); write(36, "data");
    view.setUint32(40, pcm.byteLength, true); new Uint8Array(wav, 44).set(new Uint8Array(pcm.buffer));
...
export async function getBuddyClonePreview(): Promise<{ blob: Blob; provider: string; createdAt: string } | null> {
...
export async function clearBuddyVoiceSample(): Promise<void> {
  try {
    ...
    try {
      await withTimeout(new Promise<void>((resolve, reject) => {
        const tx = db.transaction(STORE, "readwrite");
        tx.objectStore(STORE).delete(SAMPLE_KEY); tx.objectStore(STORE).delete(CLONE_PREVIEW_KEY);
        tx.oncomplete = () => resolve(); tx.onerror = () => reject(tx.error);
      }), STORAGE_TIMEOUT_MS, "[voice-storage] Clearing local voice state timed out.");
    } finally { db.close(); }
  } catch {}
}
    ...
    try {
      ...
      if (value && typeof value === "object") {
        ...
        return {
          ...
          referenceTranscript: typeof item.referenceTranscript === "string" ? item.referenceTranscript : undefined,
        };
      }
    } catch {}
  }
...
export async function markBuddyCloneVerified(provider: string) {
  ...
  saveBuddyVoiceProfile({ ...profile, mode: "clone", cloneVerified: true, cloneVerifiedAt: new Date().toISOString(), cloneProvider: provider });
}
export async function clearBuddyVoiceClone() {
  const profile = getBuddyVoiceProfile();
  await clearBuddyVoiceSample();
  saveBuddyVoiceProfile({ mode: "preset", speaker: profile.speaker || "Ryan", language: profile.language || "English", mood: profile.mood || "natural", tone: profile.tone || "conversational", cloneVerified: false });
}
