...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  // One primary attempt: never burn several full model generations just to discover a route is unavailable.
  try {
    const path = await referencePath(referenceId, body.audioBase64, audioType, primary, env);
    return await downloadAudio(
      await officialClone(primary, path, audioType, refText, text, language, requested, env, false),
      env,
  ...
  // One independent free fallback, preferring the same requested model size.
  try {
  ...
  return errorResponse(
    `Qwen voice cloning failed after two bounded attempts. ${failures.join(" | ")}`,
    502,
  );
}
