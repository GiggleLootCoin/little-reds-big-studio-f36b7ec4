...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    return await downloadAudio(await officialClone(primary, path, audioType, refText, text, language, requested, env, false), env, `Qwen3-TTS ${requested} Base full-reference`);
  } catch (error) {
  ...
  try {
    ...
    return await downloadAudio(await officialClone(fallback, path, audioType, refText, text, language, requested, env, false), env, `Qwen3-TTS fallback ${requested} Base`);
  } catch (error) {
    failures.push(error instanceof Error ? error.message : String(error));
  }
  ...
  try {
    ...
    return await downloadAudio(await chatterboxClone(chatterbox, path, audioType, text, env), env, "Chatterbox independent voice-clone fallback");
  } catch (error) {
    failures.push(error instanceof Error ? error.message : String(error));
  }
