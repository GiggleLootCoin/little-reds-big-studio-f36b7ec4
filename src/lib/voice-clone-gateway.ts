...
// Warm Worker isolates keep the uploaded reference path briefly. The browser
// sends the reference only when the cache is cold, avoiding a large upload on
...
async function cachedQwenPath(referenceId: string, audioBase64: string | undefined, audioType: string, space: string, env: Env) {
  const now = Date.now();
...
export function parseQwenSSE(stream: string): QwenSSEParseResult {
  ...
  const parseFrame = (): boolean => {
    ...
    if (currentEvent === "error" || currentEvent === "cancelled") {
      ...
      } catch { result = { kind: "error", message: data }; }
      return true;
    }
    if (currentEvent !== "complete") return false;
    let payload: unknown;
    try { payload = JSON.parse(data) as unknown; }
    catch { throw new Error("Qwen returned an invalid completed clone result."); }
    if (Array.isArray(payload)) {
      const status = payload[1];
      if (!payload[0] && typeof status === "string" && /^Error:\s*/i.test(status.trim())) {
        result = { kind: "error", message: status.trim() }; return true;
      }
      result = { kind: "audio", payload }; return true;
    }
    result = { kind: "none" }; return true;
  };
  for (const line of stream.split(/\r\n|\n|\r/)) {
    if (line.trim() === "") {
      if (parseFrame()) break;
      currentEvent = ""; dataLines = []; continue;
    }
...
async function qwenClone(space: string, path: string, audioType: string, refText: string, text: string, language: string, env: Env): Promise<string> {
  const fileData = { path, orig_name: `reference.${audioExtension(audioType)}`, mime_type: audioType || undefined, meta: { _type: "gradio.FileData" } };
  const conditioning = { ref_audio: fileData, ref_text: refText, target_text: text, language: languageName(language), use_xvector_only: false, model_size: "1.7B" };
  const start = await fetch(`${space}/gradio_api/call/generate_voice_clone`, {
    method: "POST", headers: { ...authHeaders(env), "content-type": "application/json" },
    body: JSON.stringify({ data: [conditioning.ref_audio, conditioning.ref_text, conditioning.target_text, conditioning.language, conditioning.use_xvector_only, conditioning.model_size] }),
  });
  if (!start.ok) { const detail = await start.text().catch(() => ""); throw new Error(`Qwen clone job could not start (${start.status}). ${detail.slice(0, 300)}`); }
  const started = (await start.json()) as { event_id?: string };
  if (!started.event_id) throw new Error("Qwen did not return a clone job ID.");
  const result = await fetch(`${space}/gradio_api/call/generate_voice_clone/${started.event_id}`, { headers: { ...authHeaders(env), Accept: "text/event-stream" } });
  if (!result.ok) { const detail = await result.text().catch(() => ""); throw new Error(`Qwen clone job failed (${result.status}). ${detail.slice(0, 300)}`); }
  const parsed = parseQwenSSE(await result.text());
  if (parsed.kind === "error") throw new Error(`Qwen clone job errored: ${parsed.message.slice(0, 500)}`);
  if (parsed.kind !== "audio") throw new Error("Qwen returned no completed clone audio.");
  const payload = parsed.payload;
  if (!payload[0]) throw new Error("Qwen completed without an audio artifact.");
  ...
  if (typeof audio !== "string" && audio.path) return `${space}/gradio_api/file=${String(audio.path).replace(/^\//, "")}`;
...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  let body: { referenceId?: string; audioBase64?: string; audioType?: string; refText?: string; text?: string; language?: string };
  ...
  try {
    const path = await cachedQwenPath(body.referenceId.trim(), body.audioBase64, audioType, space, env);
    const audioUrl = await qwenClone(space, path, audioType, body.refText.trim(), body.text.trim(), String(body.language || "English"), env);
    ...
    const cacheMiss = message.includes("expired from the warm server");
    return errorResponse(message, cacheMiss ? 428 : 502);
  }
}
