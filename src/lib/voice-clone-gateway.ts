...
async function officialClone(space: string, path: string, audioType: string, refText: string, text: string, language: string, modelSize: "0.6B" | "1.7B", env: Env): Promise<string> {
  const file = { path, orig_name: `reference.${audioExtension(audioType)}`, mime_type: audioType, meta: { _type: "gradio.FileData" } };
  const payload = await startAndPoll(space, "generate_voice_clone", [file, refText, text, languageName(language), false, modelSize], env);
  return audioUrl(space, payload[0]);
}

async function fallbackClone(space: string, path: string, refText: string, text: string, language: string, env: Env): Promise<string> {
  const file = { path, orig_name: "reference.wav", meta: { _type: "gradio.FileData" } };
  const payload = await startAndPoll(space, "handle_generate", ["Qwen/Qwen3-TTS-12Hz-1.7B-Base", null, file, refText, text, languageName(language), false, 200, false, false], env);
  return audioUrl(space, payload[0]);
}
...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  const fallback = String(env.QWEN_TTS_FALLBACK_SPACE_URL || "https://flausch-qwen3tts.hf.space").replace(/\/$/, "");
  ...
  try {
    const path = await referencePath(body.referenceId.trim(), body.audioBase64, audioType, primary, env);
    try { return await downloadAudio(await officialClone(primary, path, audioType, body.refText.trim(), text, language, requested, env), env, `Qwen3-TTS ${requested} Base`); }
    catch (error) { failures.push(error instanceof Error ? error.message : String(error)); }
    try { return await downloadAudio(await officialClone(primary, path, audioType, body.refText.trim(), text, language, alternate, env), env, `Qwen3-TTS ${alternate} Base`); }
    catch (error) { failures.push(error instanceof Error ? error.message : String(error)); }
  } catch (error) { failures.push(error instanceof Error ? error.message : String(error)); }

  try {
    const path = await referencePath(body.referenceId.trim(), body.audioBase64, audioType, fallback, env);
    return await downloadAudio(await fallbackClone(fallback, path, body.refText.trim(), text, language, env), env, "Qwen3-TTS 1.7B free fallback");
  } catch (error) {
