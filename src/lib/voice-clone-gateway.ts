...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    return await downloadAudio(
      await officialClone(fallback, path, audioType, refText, text, language, requested, env, false),
      env,
  ...
  return errorResponse(`Qwen voice cloning failed after two bounded attempts. ${failures.join(" | ")}`, 502);
}
