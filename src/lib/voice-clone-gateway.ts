...
async function uploadReference(space: string, audio: Blob, env: Env): Promise<string> {
  ...
  if (!response.ok)
    throw new Error(
      `Reference upload failed (${response.status}). ${(await response.text()).slice(0, 240)}`,
    );
...
function audioUrl(space: string, value: unknown): string {
  ...
  if (!value || typeof value !== "object")
    throw new Error("Provider returned no downloadable audio artifact.");
...
async function startAndPoll(
  ...
  if (!start.ok)
    throw new Error(
      `${endpoint} start failed (${start.status}). ${(await start.text()).slice(0, 300)}`,
    );
  ...
  if (!result.ok)
    throw new Error(
      `${endpoint} job failed (${result.status}). ${(await result.text()).slice(0, 300)}`,
    );
...
async function chatterboxClone(
  space: string,
  path: string,
  audioType: string,
  text: string,
...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const path = await referencePath(referenceId, body.audioBase64, audioType, chatterbox, env);
    return await downloadAudio(
      await chatterboxClone(chatterbox, path, audioType, text, env),
      env,
  ...
  return errorResponse(
    `Voice cloning failed after bounded independent attempts. ${failures.join(" | ")}`,
    502,
  );
}
