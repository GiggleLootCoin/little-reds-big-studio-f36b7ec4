...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  const fallback = String(
    env.QWEN_TTS_FALLBACK_SPACE_URL || "https://flausch-qwen3tts.hf.space",
  ).replace(/\/$/, "");
  ...
  try {
    ...
    return await downloadAudio(
      await fallbackClone(fallback, path, body.refText.trim(), text, language, env),
      env,
      "Qwen3-TTS 1.7B free fallback",
    );
  } catch (error) {
    failures.push(error instanceof Error ? error.message : String(error));
    return errorResponse(
