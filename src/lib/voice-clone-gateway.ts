...
async function downloadAudio(url: string, env: Env, provider: string): Promise<Response> {
  ...
  headers.set("cache-control", "no-store"); headers.set("x-clone-provider", provider); headers.delete("x-clone-verified");
  return new Response(response.body, { status: 200, headers });
}

async function cloneWithReferenceRefresh(
  space: string,
  referenceId: string,
  audioBase64: string | undefined,
  audioType: string,
  ...
  modelSize: "0.6B" | "1.7B",
  env: Env,
  clone: (path: string) => Promise<string>,
): Promise<string> {
...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  try {
    return await downloadAudio(
      await cloneWithReferenceRefresh(primary, referenceId, body.audioBase64, audioType, refText, text, language, requested, env, (path) => officialClone(primary, path, audioType, refText, text, language, requested, env, false)),
      env, `Qwen3-TTS ${requested} Base full-reference`,
    );
  } catch (error) { failures.push(error instanceof Error ? error.message : String(error)); }

  try {
    return await downloadAudio(
      await cloneWithReferenceRefresh(fallback, referenceId, body.audioBase64, audioType, refText, text, language, requested, env, (path) => officialClone(fallback, path, audioType, refText, text, language, requested, env, false)),
      env, `Qwen3-TTS fallback ${requested} Base`,
    );
  } catch (error) { failures.push(error instanceof Error ? error.message : String(error)); }

  try {
    return await downloadAudio(
      await cloneWithReferenceRefresh(chatterbox, referenceId, body.audioBase64, audioType, refText, text, language, requested, env, (path) => chatterboxClone(chatterbox, path, audioType, text, env)),
      env, "Chatterbox independent voice-clone fallback",
    );
  } catch (error) { failures.push(error instanceof Error ? error.message : String(error)); }

  if (failures.every((message) => message === "REFERENCE_REFRESH_REQUIRED")) {
    return errorResponse("The saved voice reference expired from the warm server. Please retry once to refresh it.", 428);
  }
