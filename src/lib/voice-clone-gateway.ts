...
export function parseQwenSSE(stream: string): QwenSSEParseResult {
  ...
  const parseFrame = (): boolean => {
    ...
    if (currentEvent === "error" || currentEvent === "cancelled") {
      try { const parsed = JSON.parse(data) as unknown; result = { kind: "error", message: typeof parsed === "string" ? parsed : JSON.stringify(parsed) }; }
    ...
    if (Array.isArray(payload)) {
      ...
      if (!payload[0] && typeof status === "string" && /^Error:\s*/i.test(status.trim())) { result = { kind: "error", message: status.trim() }; return true; }
      result = { kind: "audio", payload }; return true;
    }
    result = { kind: "none" }; return true;
  };
  for (const line of stream.split(/\r\n|\n|\r/)) {
    if (line.trim() === "") { if (parseFrame()) break; currentEvent = ""; dataLines = []; continue; }
...
async function qwenClone(space: string, path: string, audioType: string, refText: string, text: string, language: string, modelSize: "0.6B" | "1.7B", env: Env): Promise<string> {
  ...
  const conditioning = { ref_audio: fileData, ref_text: refText, target_text: text, language: languageName(language), use_xvector_only: false, model_size: modelSize };
...
export async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const cached = await cachedQwenPath(body.referenceId.trim(), body.audioBase64, audioType, space, env);
    const audioUrl = await qwenClone(space, cached.path, audioType, body.refText.trim(), body.text.trim().replace(/\s+/g, " ").slice(0, 220), String(body.language || "English"), modelSize, env);
