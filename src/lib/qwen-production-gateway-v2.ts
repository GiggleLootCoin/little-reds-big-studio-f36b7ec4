...
const extensionFor = (mime: string) =>
  mime.toLowerCase().includes("mpeg")
    ? "mp3"
    : mime.toLowerCase().includes("mp4")
      ? "m4a"
      : mime.toLowerCase().includes("webm")
        ? "webm"
        : mime.toLowerCase().includes("ogg")
          ? "ogg"
          : "wav";
const decodeBase64 = (value: string) =>
  Uint8Array.from(atob(value.replace(/^data:[^,]+,/, "").replace(/\s/g, "")), (c) =>
    c.charCodeAt(0),
  );
const gatewayError = (message: string, status = 502) =>
  Response.json(
...
function findAudio(value: unknown): unknown {
  ...
  for (const key of ["url", "path", "audio", "file", "fileData", "output", "data"]) {
...
function parseQwenSSE(stream: string): { audio?: unknown; error?: string } {
  ...
  const flush = () => {
    ...
    try {
      ...
      if (event === "error" || event === "cancelled") terminalError = payload;
      return;
    }
    ...
    if (Array.isArray(parsed)) {
      ...
      if (found !== undefined) audio = found;
        ...
        const status = String((parsed as Record<string, unknown>).status || "");
        if (/^Error:/i.test(status)) terminalError = status;
      }
    }
  };
...
async function uploadReference(space: string, bytes: Uint8Array, mime: string, env: Env) {
  ...
  if (!response.ok)
    throw new Error(
      `Qwen reference upload failed (${response.status}). ${(await response.text()).slice(0, 300)}`,
    );
  const result = (await response.json()) as unknown;
  if (Array.isArray(result) && typeof result[0] === "string") return result[0];
  if (Array.isArray(result)) {
    const path = findAudio(result[0]);
    if (typeof path === "string") return path;
  }
  throw new Error("Qwen returned no uploaded reference path.");
}
async function getReferencePath(
  referenceId: string,
  ...
  referenceCache.set(referenceId, { path, expires: Date.now() + REFERENCE_TTL_MS });
  return path;
}
async function generateOnce(
  ...
  const file = {
    path: referencePath,
    orig_name: `reference.${extensionFor(input.audioType)}`,
    mime_type: input.audioType,
  ...
  const start = await fetch(`${space}/gradio_api/call/generate_voice_clone`, {
    ...
    body: JSON.stringify({
      data: [file, input.refText, input.text, languageName(input.language), false, input.modelSize],
    }),
  });
  if (!start.ok)
    throw new Error(
      `Qwen ${input.modelSize} clone job could not start (${start.status}). ${(await start.text()).slice(0, 400)}`,
    );
  const started = (await start.json()) as { event_id?: string };
  if (!started.event_id) throw new Error(`Qwen ${input.modelSize} did not return a clone job ID.`);
  ...
  if (!result.ok)
    throw new Error(
      `Qwen ${input.modelSize} clone job failed (${result.status}). ${(await result.text()).slice(0, 400)}`,
    );
  ...
  if (parsed.audio === undefined)
    throw new Error(`Qwen ${input.modelSize}: no completed clone audio.`);
...
export async function handleProductionQwenVoiceClone(request: Request, env: Env) {
  ...
  try {
    ...
    return gatewayError("The clone request was not valid JSON.", 400);
  }
  if (!body.referenceId?.trim()) return gatewayError("A voice reference ID is required.", 400);
  if (!body.refText?.trim())
    return gatewayError("The exact transcript of the reference recording is required.", 400);
  ...
  try {
    const referencePath = await getReferencePath(
    ...
    const firstSize = requestedSize,
      secondSize = requestedSize === "0.6B" ? "1.7B" : "0.6B";
    const common = {
      audioType,
    ...
    try {
      generatedUrl = await generateOnce(
        space,
        referencePath,
        { ...common, modelSize: firstSize },
        env,
      );
    } catch (firstError) {
      generatedUrl = await generateOnce(
        space,
        referencePath,
        { ...common, modelSize: secondSize },
        env,
      ).catch((secondError) => {
        throw new Error(
          `Qwen clone failed on both Base sizes. ${firstSize}: ${firstError instanceof Error ? firstError.message : String(firstError)} | ${secondSize}: ${secondError instanceof Error ? secondError.message : String(secondError)}`,
        );
      });
    }
    ...
    headers.set("x-clone-provider", "Qwen3-TTS Base");
    headers.delete("x-clone-verified");
    return new Response(generated.body, { status: 200, headers });
  } catch (error) {
    return gatewayError(error instanceof Error ? error.message : "Qwen voice cloning failed.");
  }
}
