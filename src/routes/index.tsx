    ...
    <div className="relative min-h-[100svh] overflow-x-clip">
      ...
      <header className="sticky top-0 z-30 border-b border-border/50 bg-background/80 shadow-[0_12px_40px_oklch(0_0_0_/_0.2)] backdrop-blur-2xl supports-[backdrop-filter]:bg-background/60">
        <div className="mx-auto flex min-h-16 w-full max-w-6xl items-center justify-between gap-3 px-3 py-2 sm:px-5 sm:py-3">
          <button type="button" aria-label="Go to Studio home" onClick={() => go("home")} className="min-w-0 rounded-2xl p-1.5 text-left transition-transform active:scale-[0.98]">
            <StudioLogo compact={false} className="[&>svg]:size-10 sm:[&>svg]:size-12" />
          </button>
          <div className="hidden items-center gap-2 sm:flex">
            <Chip>Free/open first</Chip>
            <span className="rounded-full border border-primary/25 bg-primary/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-primary">Buddy online</span>
          </div>
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.65fr)] lg:items-start">
              ...
              <section className="rounded-2xl border border-border/60 bg-background/45 p-4 backdrop-blur-md sm:p-5">
                ...
                <p className="mt-1 text-xs leading-5 text-muted-foreground">Start anywhere. Buddy keeps the workflow moving while the engine deck gives you direct free fallbacks.</p>
                <div className="mt-4 flex flex-wrap gap-2"><Chip>Android-first</Chip><Chip>Free/open</Chip><Chip>Buddy-led</Chip></div>
              </section>
            </div>
            <section className="space-y-3">
              <div><h2 className="font-display text-sm font-semibold uppercase tracking-[0.2em] text-foreground">Your next move</h2><p className="mt-1 text-xs text-muted-foreground">Pick a goal. The machinery stays backstage.</p></div>
