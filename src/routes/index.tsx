  ...
  return (
    <div className="relative min-h-[100svh] overflow-x-clip">
      ...
      <main className="mx-auto w-full max-w-6xl px-3 pb-28 pt-3 sm:px-5 sm:pb-16 sm:pt-6 lg:px-8">
        ...
        {tab === "mix" && (
          <div className="space-y-3"><UploadPanel /><QRangePanel /><LabPanel /></div>
        )}
        {tab === "video" && (
          <div className="space-y-3"><StoryboardPanel /><VideoPanel /><SeoPanel /></div>
        )}
        {tab === "community" && (
          <div className="space-y-3"><SpotlightPanel /><ProfilePanel /><OfficialStationPanel /><AccessPanel /></div>
        )}
      ...
      <nav aria-label="Studio sections" className="fixed inset-x-0 bottom-0 z-40 border-t border-border/60 bg-background/95 pb-[env(safe-area-inset-bottom)] shadow-[0_-12px_40px_oklch(0_0_0_/_0.28)] backdrop-blur-2xl sm:hidden">
        <div className="mx-auto grid max-w-lg grid-cols-5">
          {TABS.map((t) => {
            ...
            return (
              <button key={t.id} type="button" aria-current={active ? "page" : undefined} onClick={() => go(t.id)} className={cn("flex min-h-14 flex-col items-center justify-center gap-1 px-1 transition-colors active:bg-primary/10", active ? "text-primary" : "text-muted-foreground")}>
                <span className={cn("flex size-8 items-center justify-center rounded-xl", active && "bg-primary/12")}><Icon aria-hidden className="size-5" /></span>
