...
import { getStoredSession, updateProfile } from "@/lib/supabase-rest";
...
export function SpotlightPanel() {
  return (
    <Panel eyebrow="Module 12" title="Artist Spotlight — Local" icon={<Heart className="size-5" />}>
      <Note>
        Your creator feed is local-first. Finished work can be saved to your account when signed in
        and shared through your normal publishing platform.
      </Note>
    </Panel>
  );
}
  ...
  if (!profile)
    return (
      <Panel eyebrow="Module 13" title="Creator Profile" icon={<UserCircle2 className="size-5" />}>
        <Note>Loading account profile…</Note>
      </Panel>
    );
  return (
    <Panel eyebrow="Module 13" title="Creator Profile" icon={<UserCircle2 className="size-5" />}>
      ...
      <p className="text-xs text-muted-foreground">
        Your profile is stored in Supabase. Trial and membership state are server-authoritative.
      </p>
      <StudioButton
        className="w-full"
        disabled={saving}
          ...
          void (async () => {
            ...
            try {
              const next = await updateProfile(
                profile.id,
                { display_name: name, avatar_url: profile.avatar_url },
                s.access_token,
              );
...
export function AccessPanel() {
  return (
    <Panel eyebrow="Account" title="Account & Privacy" icon={<Users className="size-5" />}>
      <Readout label="Account" value="Supabase authentication" />
      <Readout label="AI API keys" value="None required for free routes" />
      <Readout label="Entitlement" value="Server-authoritative" />
      <Note>
        Projects and account data are scoped to the signed-in user. Browser storage is used only for
        lightweight UI/session continuity.
      </Note>
    </Panel>
  );
}
...
export function SeoPanel() {
  return (
    <Panel eyebrow="Module 15" title="YouTube SEO Workspace" icon={<Plug className="size-5" />}>
      <Note>
        SEO generation stays inside Buddy&apos;s orchestration layer rather than exposing provider
        machinery.
      </Note>
    </Panel>
  );
...
function runnerUrlForPlugin(plugin: PublicPlugin) {
  ...
  return (
    FREE_RUNNERS.find((runner) => runner.capabilities.includes(capability))?.url ??
    plugin.projectUrl
  );
}
  ...
  return (
    <Panel eyebrow="Open Models" title="No-API Model Catalog" icon={<Plug className="size-5" />}>
      <p className="text-sm text-muted-foreground">
        Open/free engines are selected backstage. This diagnostic catalog is for transparency;
        normal creation controls do not require model selection.
      </p>
      <div className="grid gap-2">
        {plugins.map((p) => {
...
export function SupportPanel() {
  return (
    <Panel eyebrow="Free by design" title="Studio Support" icon={<Plug className="size-5" />}>
      <Note>
        Little Red&apos;s Big Studio prioritizes open/free execution. Core creation does not require
        a paid AI gateway or provider API key.
      </Note>
    </Panel>
  );
}
