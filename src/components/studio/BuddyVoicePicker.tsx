...
const FRIENDLY_CLONE_FAILURE = "I couldn't make that voice sample just yet. Your sample is safe. Want to try again?";
const FRIENDLY_VOICE_FAILURE = "Buddy couldn't get that voice ready just yet. Want to try another voice?";
...
export function BuddyVoicePicker() {
  ...
  const update = (patch: Partial<typeof profile>) => {
    ...
    setStatus(next.mode === "clone" ? "Your saved voice is selected for Buddy." : `${label} is selected for Buddy.`);
  };
  const runClone = async (sample: Blob, current = getBuddyVoiceProfile()) => {
    const result = await runStudioJob("voice-clone", {
      ...
      refText: current.referenceTranscript || "", target_text: CLONE_TEXT,
      text: CLONE_TEXT, language: current.language || "Auto",
      use_xvector_only: !current.referenceTranscript, model_size: "1.7B",
      mood: current.mood, tone: current.tone,
    }, () => undefined);
  ...
  const uploadClone = async (file: File) => {
    ...
    try {
      ...
      } catch { /* clone can continue without a transcript */ }
      ...
      try {
        ...
        setStatus(FRIENDLY_CLONE_FAILURE);
      }
    } catch (error) {
      setStatus(error instanceof Error ? `I couldn't save that voice sample. ${error.message}` : "I couldn't save that voice sample.");
  ...
  const preview = async () => {
    ...
    try {
      ...
      if (current.mode === "clone") {
        ...
        const url = await runClone(sample, current), audio = new Audio(url);
        await audio.play(); setStatus("Buddy's cloned voice is working."); return;
      }
      const result = await runStudioJob("tts", {
        text: CLONE_TEXT, target_text: CLONE_TEXT, language: current.language || "English",
        speaker: current.speaker, mood: current.mood, tone: current.tone,
        instruct: current.mood === "natural" && current.tone === "conversational" ? "" : `Speak naturally with a ${current.mood || "natural"} mood and ${current.tone || "conversational"} tone.`,
      }, () => undefined);
      if (!result.url) throw new Error("No playable voice audio was returned.");
      const audio = new Audio(result.url); await audio.play(); setStatus("Buddy's selected voice is working.");
    } catch { setStatus(FRIENDLY_VOICE_FAILURE); }
    finally { setBusy(false); }
  };
  return (
