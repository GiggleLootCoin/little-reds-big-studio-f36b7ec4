...
export function BuddyVoicePicker() {
  ...
  const setGeneratedAudio = (url: string) => {
    if (audioUrlRef.current && audioUrlRef.current !== url && audioUrlRef.current.startsWith("blob:")) URL.revokeObjectURL(audioUrlRef.current);
  ...
  const previewPreset = () => {
    ...
    setStatus(`✓ ${displaySpeaker(speaker)} preview loaded — press Play below.`);
  };
