...
import { createRealVoiceClone } from "@/lib/real-voice-clone";
...
const CLONE_TEXT = "Hi. I'm Buddy. This is a voice-clone test. The voice you supplied is speaking these words.";
const FAILURE = "Buddy couldn't get that voice ready just yet. Try Generate + verify again.";
export function BuddyVoicePicker() {
  ...
  const allVoices = useMemo(() => [...BUDDY_VOICE_PRESETS.map((v) => ({ ...v, family: "Buddy Originals" })), ...BUDDY_EXPANDED_VOICES.map((v) => ({ ...v, family: "Aura Studio — 40 distinct English voices" }))], []);
  const update = (patch: Partial<typeof profile>) => {
    ...
    setStatus(next.mode === "clone" ? next.cloneVerified ? "✓ Your verified clone is ready for Buddy." : "Your reference is saved; check the transcript, then generate the clone." : `${voice?.label || next.speaker} selected — ${voice?.note || "ready to test"}.`);
  };
  const runClone = async (sample: Blob, current = getBuddyVoiceProfile()) => {
    const transcript = String(current.referenceTranscript || "").trim();
    if (!transcript) throw new Error("Enter or correct the exact transcript of the reference recording first.");
    const result = await createRealVoiceClone(sample, transcript, CLONE_TEXT, current.language || "English");
  ...
  const uploadClone = async (file: File) => {
    ...
    setBusy(true); setStatus("Saving your reference and transcribing it…");
    try {
      ...
      let transcript = "";
      try { transcript = artifactText((await runStudioJob("speech-to-text", { audio: file }, setStatus)).value).trim(); } catch { transcript = ""; }
      ...
      setStatus(transcript ? "Reference saved. Check/correct the transcript below, then tap Generate + verify." : "Reference saved, but transcription was unavailable. Enter the exact transcript below, then tap Generate + verify.");
    } catch (error) { setStatus(`${FAILURE} ${error instanceof Error ? error.message : "The recording could not be saved."}`); }
    finally { setBusy(false); }
  };
  const test = async () => {
    setBusy(true); setStatus("Generating a real voice clone…");
    try {
      const current = getBuddyVoiceProfile();
      if (current.mode === "clone") {
        const sample = await getBuddyVoiceSample(); if (!sample) throw new Error("Your saved voice reference is missing. Record it again.");
        const result = await runClone(sample, current); const player = new Audio(result.url); await player.play(); await markBuddyCloneVerified(result.provider); setProfile(getBuddyVoiceProfile()); setStatus(`✓ REAL CLONE VERIFIED — ${result.provider}.`); return;
      }
      ...
      if (!result.url) throw new Error("No playable audio returned."); const player = new Audio(result.url); await player.play(); setStatus(`✓ ${allVoices.find((v) => v.id === current.speaker)?.label || current.speaker} is working.`);
