...
export function BuddyVoicePicker() {
  ...
  const uploadClone = async (file: File) => {
    ...
    try { await saveReference(file); } catch (error) { setStatus(`${FAILURE} ${error instanceof Error ? error.message : "The recording could not be saved."}`); } finally { setBusy(false); }
  };
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => { stream.getTracks().forEach((track) => track.stop()); const blob = new Blob(chunksRef.current, { type: recorder.mimeType || "audio/webm" }); const file = new File([blob], `voice-sample-${Date.now()}.webm`, { type: blob.type }); await uploadClone(file); };
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/70 p-3 shadow-sm">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button type="button" onClick={() => update({ mode: "preset" })} className={`rounded-xl border px-3 py-2 text-left text-xs ${profile.mode === "preset" ? "border-primary bg-primary/10" : "border-border bg-background/40"}`}><Play className="mb-1 size-4 text-primary" /> Preset Voices<span className="mt-1 block text-[9px] text-muted-foreground">Real selectable speaker IDs</span></button>
      ...
      {profile.mode === "preset" ? (
        <div className="mt-2 rounded-xl border border-border bg-background/40 p-2">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2"><StudioButton type="button" className="w-full justify-center" onClick={() => void previewPreset()} disabled={busy}><Volume2 className="size-4" /> Preview Voice</StudioButton><button type="button" onClick={() => update({ mode: "preset", speaker: presetCandidate })} disabled={busy} className="rounded-xl border border-primary bg-primary/10 px-3 py-2 text-xs font-semibold"><CheckCircle2 className="mr-1 inline size-4" /> Use This Voice</button></div>
