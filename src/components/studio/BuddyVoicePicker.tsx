...
import { clearBuddyVoiceClone, getBuddyVoiceProfile, getBuddyVoiceSample, markBuddyCloneVerified, saveBuddyVoiceProfile } from "@/lib/buddy-voice";
...
export function BuddyVoicePicker() {
  ...
  const allVoices = useMemo(() => [{ id: "Red", label: "Red — The Original Voice of Buddy", note: "Your real verified personal voice clone", nativeLanguage: "English", languages: ["English"], character: "Little Red’s own voice — the required Buddy voice.", family: "Buddy's Original Voice" }, ...BUDDY_VOICE_PRESETS.map((v) => ({ ...v, family: "Buddy Originals" })), ...BUDDY_EXPANDED_VOICES.map((v) => ({ ...v, family: "Aura Studio — 40 distinct English voices" }))], []);
  const setGeneratedAudio = (url: string) => { if (audioUrlRef.current && audioUrlRef.current !== url) URL.revokeObjectURL(audioUrlRef.current); audioUrlRef.current = url; setAudioUrl(url); };
  const update = (patch: Partial<typeof profile>) => {
    ...
    setStatus(canonicalNext.mode === "clone" || canonicalNext.speaker === "Red" ? canonicalNext.cloneVerified ? "✓ Your verified clone is ready for Buddy." : "Your sample is saved. Tap Generate My Voice Clone." : `${voice?.label || displaySpeaker(speaker)} selected — ${voice?.note || "ready to test"}.`);
  };
  const saveReference = async (file: File) => {
    ...
    const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(new Error("The recording could not be read.")); reader.readAsDataURL(file); });
    ...
    saveBuddyVoiceProfile(next); setProfile(next); setStatus("Voice sample saved with its exact transcript. Tap Generate My Voice Clone.");
  };
  const uploadClone = async (file: File) => { setBusy(true); setStatus("Saving your voice sample locally…"); try { await saveReference(file); } catch (error) { setStatus(`${FAILURE} ${error instanceof Error ? error.message : "The recording could not be saved."}`); } finally { setBusy(false); } };
  const startRecording = async () => {
    if (!navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) { setStatus("Voice recording is not supported by this browser. Upload an audio sample instead."); return; }
    try {
      ...
      recorder.onstop = async () => { stream.getTracks().forEach((track) => track.stop()); const blob = new Blob(chunksRef.current, { type: recorder.mimeType || "audio/webm" }); await uploadClone(new File([blob], `voice-sample-${Date.now()}.webm`, { type: blob.type })); };
      recorderRef.current = recorder; recorder.start(); setRecording(true); setStatus(`Read this sentence naturally: “${REFERENCE_TRANSCRIPT}”`);
    } catch { setStatus(`${FAILURE} Microphone access was not granted.`); }
  };
  ...
  const test = async () => {
    ...
    try {
      ...
      if (current.mode === "clone" || current.speaker === "Red") {
        const sample = await getBuddyVoiceSample(); if (!sample) throw new Error("Your voice sample is missing. Upload or record it again.");
        const referenceTranscript = current.referenceTranscript || REFERENCE_TRANSCRIPT;
        const result = await runStudioJob("voice-clone", { refAudio: sample, referenceAudio: sample, audio: sample, referenceTranscript, refText: referenceTranscript, target_text: CLONE_TEXT, text: CLONE_TEXT, language: current.language || "English" }, setStatus);
        if (!result.url) throw new Error("The clone engine returned no playable audio."); setGeneratedAudio(result.url); await markBuddyCloneVerified(result.provider); setProfile(getBuddyVoiceProfile()); setStatus("✓ REAL VOICE CLONE READY — press Play on the audio player below."); return;
      }
      ...
      if (!result.url) throw new Error("The voice engine returned no playable audio."); setGeneratedAudio(result.url); setStatus("✓ Voice sample generated — press Play below.");
    } catch (error) { setStatus(`${FAILURE} ${error instanceof Error ? error.message : "The voice engine failed."}`); } finally { setBusy(false); }
  };
  const removeClone = async () => { await clearBuddyVoiceClone(); setGeneratedAudio(""); const next = getBuddyVoiceProfile(); setProfile(next); setStatus("Voice clone removed. Upload or record a new sample to continue."); };
  return (
