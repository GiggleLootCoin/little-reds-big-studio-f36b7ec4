...
const FAILURE =
  "Buddy couldn't get that voice ready just yet. Try Test again or choose another voice.";
...
export function BuddyVoicePicker() {
  ...
  const update = (patch: Partial<typeof profile>) => {
    ...
    setStatus(
      next.mode === "clone"
        ? next.cloneVerified
          ? "✓ Red's verified clone is ready for Buddy."
          : "Red's reference is saved; the clone still needs a real generated sample."
  ...
  const uploadClone = async (file: File) => {
    ...
    if (file.size > 3_500_000) return setStatus("Use a clear 2–30 second recording under 3.5 MB.");
    setBusy(true);
    setStatus("Saving Red's reference and generating a real clone…");
    try {
      ...
      if (!Number.isFinite(duration) || duration < 2 || duration > 30)
        throw new Error("Use a clear voice recording between 2 and 30 seconds.");
      ...
      if (!saved) throw new Error("The saved reference could not be retrieved.");
      ...
      await player.play();
      setStatus(
        `✓ REAL CLONE VERIFIED — ${result.provider}. Red's voice is now a usable Buddy voice.`,
      );
    } catch (error) {
      setStatus(
  ...
  const test = async () => {
    setBusy(true);
    setStatus("Testing the selected voice…");
    try {
      const current = getBuddyVoiceProfile();
      if (current.mode === "clone") {
        const sample = await getBuddyVoiceSample();
        if (!sample) throw new Error("Red's saved voice reference is missing.");
  ...
  const removeClone = async () => {
    ...
    setProfile(next);
    setStatus("Red's clone removed. Buddy is back on preset voices.");
  };
  return (
    <div className="rounded-2xl border border-primary/30 bg-primary/5 p-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              50 distinct preset identities, multilingual routing, and Red's real clone.
            </p>
          </div>
        </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        ...
        >
          ...
          <span className="mt-1 block text-[9px] text-muted-foreground">
            2–30 sec reference • real verification
          </span>
          <input
            className="sr-only"
            type="file"
        ...
        <div className="mt-2 rounded-xl border border-primary/30 bg-background/60 p-3">
          ...
          <p className="mt-1 text-[10px] text-muted-foreground">
            {profile.cloneVerified
              ...
              : "The upload alone does not count as a clone. Buddy will only show READY after generated audio is returned and played."}
          </p>
