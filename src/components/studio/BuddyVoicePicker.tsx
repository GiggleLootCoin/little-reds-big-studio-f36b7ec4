...
export function BuddyVoicePicker() {
  ...
  const previewPreset = () => {
    ...
    const previewUrl = getStoredPresetPreview(speaker);
    if (!previewUrl) {
      setStatus("This preset does not have a stored preview audio asset yet.");
      return;
    }
    setPreviewVoice(speaker);
    setGeneratedAudio(previewUrl);
    setStatus(`✓ Stored ${allVoices.find((v) => v.id === speaker)?.label || speaker} preview ready — press Play below.`);
  };
  ...
  const handleGenerateClick = () => void test();
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/70 p-3 shadow-sm">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button type="button" onClick={() => update({ mode: "preset" })} className={`rounded-xl border px-3 py-2 text-left text-xs ${profile.mode === "preset" ? "border-primary bg-primary/10" : "border-border bg-background/40"}`}>
      ...
      {profile.mode === "preset" ? (
        <div className="mt-2 rounded-xl border border-border bg-background/40 p-2">
          <select value={presetCandidate} onChange={(e) => { setPresetCandidate(e.target.value); setPreviewVoice(null); setStatus("Preset selected for preview — it is not committed until you use this voice."); }} className="w-full rounded-xl border border-border bg-background/70 px-3 py-2 text-xs">
            {["Buddy's Original Voice", "Buddy Originals", "Aura Studio — 40 distinct English voices"].map((family) => (
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <StudioButton type="button" className="w-full justify-center" onClick={previewPreset} disabled={busy}>
              <Volume2 className="size-4" /> Preview Voice
            </StudioButton>
            <button type="button" onClick={() => update({ mode: "preset", speaker: presetCandidate })} disabled={busy} className="rounded-xl border border-primary bg-primary/10 px-3 py-2 text-xs font-semibold">
              <CheckCircle2 className="mr-1 inline size-4" /> Use This Voice
            </button>
          </div>
          {audioUrl && previewVoice === presetCandidate && <div className="mt-2 rounded-xl border border-primary/30 bg-background/70 p-2"><p className="mb-2 text-[10px] font-semibold text-primary">Preset voice preview</p><audio className="w-full" controls preload="metadata" src={audioUrl} /></div>}
        ...
        <div className="mt-2 rounded-xl border border-primary/30 bg-background/60 p-3">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <label className="cursor-pointer rounded-xl border border-border bg-background/50 px-3 py-3 text-center text-xs font-semibold"><Volume2 className="mx-auto mb-1 size-5 text-primary" /> Upload Sample<span className="mt-1 block text-[9px] font-normal text-muted-foreground">30–60 seconds</span><input className="sr-only" type="file" accept="audio/*" disabled={busy || recording} onChange={(e) => { const f = e.target.files?.[0]; if (f) void uploadClone(f); e.currentTarget.value = ""; }} /></label>
