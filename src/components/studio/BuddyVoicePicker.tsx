...
export function BuddyVoicePicker() {
  ...
  const allVoices = useMemo(() => [
    { id: "Red", label: "Red — The Original Voice of Buddy", note: "Your real verified personal voice clone", nativeLanguage: "English", languages: ["English"], character: "Little Red’s own voice — the required Buddy voice.", family: "Buddy's Original Voice" },
  ...
  const previewPreset = async () => {
    ...
    try {
      ...
      if (stored) {
        setGeneratedAudio(stored); const player = new Audio(stored); player.preload = "auto";
        try { await player.play(); setStatus(`✓ ${displaySpeaker(speaker)} preview playing.`); } catch { setStatus("✓ Preview loaded — press Play on the audio player below if Android blocks automatic playback."); }
        return;
      }
      ...
      const blob = await response.blob(); if (!blob.size) throw new Error("The selected preset returned empty audio.");
      const url = URL.createObjectURL(blob); setGeneratedAudio(url); const player = new Audio(url); player.preload = "auto";
  ...
  const test = async () => {
    ...
    try {
      ...
      if (current.mode === "clone" || current.speaker === "Red") {
        ...
        setGeneratedAudio(result.url); await markBuddyCloneVerified(result.provider); setProfile(getBuddyVoiceProfile()); setStatus("✓ REAL VOICE CLONE READY — press Play on the audio player below."); return;
      }
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/70 p-3 shadow-sm">
      ...
      {profile.mode === "preset" ? <div className="mt-2 rounded-xl border border-border bg-background/40 p-2">
      ...
      </div> : <div className="mt-2 rounded-xl border border-primary/30 bg-background/60 p-3">
