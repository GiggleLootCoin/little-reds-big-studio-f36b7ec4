...
export function BuddyVoicePicker() {
  ...
  const update = (patch: Partial<typeof profile>) => {
    ...
    setStatus(next.mode === "clone" ? next.cloneVerified ? "✓ Your verified clone is ready for Buddy." : "Your sample is saved. Tap Generate My Voice Clone." : `${voice?.label || next.speaker} selected — ${voice?.note || "ready to test"}.`);
  };
  ...
  const handleGenerateClick = () => { console.info("[BuddyVoiceDiagnostic] GENERATE_CLICKED"); void test(); };
    ...
    <div className="rounded-2xl border border-border bg-card/70 p-3 shadow-sm">
      ...
      {profile.mode === "preset" ? <select value={profile.speaker} onChange={(e) => update({ speaker: e.target.value })} className="mt-2 w-full rounded-xl border border-border bg-background/70 px-3 py-2 text-xs">{["Buddy Originals", "Aura Studio — 40 distinct English voices"].map((family) => <optgroup key={family} label={family}>{allVoices.filter((v) => v.family === family).map((voice) => <option key={voice.id} value={voice.id}>{voice.label} — {voice.note}</option>)}</optgroup>)}</select> : (
        <div className="mt-2 rounded-xl border border-primary/30 bg-background/60 p-3">
          ...
          <p role="status" aria-live="polite" className="mt-3 rounded-xl border border-border/70 bg-background/50 px-3 py-2 text-[10px] leading-relaxed text-muted-foreground">{status}</p>
          <StudioButton type="button" className="mt-3 w-full justify-center" onClick={handleGenerateClick} disabled={busy || recording} aria-busy={busy}><Mic2 className="size-4" /> {busy ? "Generating Your Voice Clone…" : "Generate My Voice Clone"}</StudioButton>
          {audioUrl && <div className="mt-3 rounded-xl border border-primary/30 bg-background/70 p-2"><p className="mb-2 text-[10px] font-semibold text-primary">Your generated voice sample</p><audio className="w-full" controls preload="metadata" src={audioUrl} /></div>}
      ...
      <div className="mt-2 grid grid-cols-3 gap-2">
        ...
        <label className="text-[10px] font-semibold text-muted-foreground">Tone<select value={profile.tone || "conversational"} onChange={(e) => update({ tone: e.target.value })} className="mt-1 w-full rounded-xl border border-border bg-background/70 px-2 py-2 text-xs font-normal text-foreground">{BUDDY_TONES.map((tone) => <option key={tone.id} value={tone.id}>{tone.label}</option>)}</select></label>
      </div>
    </div>
  );
}
