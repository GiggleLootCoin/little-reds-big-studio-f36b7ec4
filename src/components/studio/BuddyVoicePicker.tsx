...
const CLONE_TEXT =
  "Hi. I'm Buddy from Little Red's Big Studio. This is a voice-clone test. The voice you supplied is speaking these words.";
const FAILURE =
...
export function BuddyVoicePicker() {
    ...
    setStatus(
      next.mode === "clone"
        ? next.cloneVerified
          ...
          : "Your reference is saved; the clone still needs a real generated sample."
  ...
  const uploadClone = async (file: File) => {
    ...
    setBusy(true);
    setStatus("Saving your reference and generating a real clone…");
    try {
      ...
      try {
        ...
        /* x-vector fallback remains available */
      }
      const next = {
        ...getBuddyVoiceProfile(),
        mode: "clone" as const,
      ...
      await player.play();
      setStatus(
        `✓ REAL CLONE VERIFIED — ${result.provider}. Your clone is now a usable Buddy voice.`,
      );
    } catch (error) {
      setStatus(
  ...
  const test = async () => {
    ...
    try {
      ...
      if (current.mode === "clone") {
        ...
        if (!sample) throw new Error("Your saved voice reference is missing.");
  ...
  return (
    <div className="rounded-2xl border border-primary/30 bg-primary/5 p-3">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        ...
        >
          <Mic2 className="mb-1 size-4 text-primary" /> Clone your voice
          <span className="mt-1 block text-[9px] text-muted-foreground">
        ...
        <div className="mt-2 rounded-xl border border-primary/30 bg-background/60 p-3">
          ...
          <p className="mt-1 text-[10px] text-muted-foreground">
            {profile.cloneVerified
              ...
              : "The upload alone does not count as a clone. Buddy shows READY only after generated audio is returned and played."}
          </p>
