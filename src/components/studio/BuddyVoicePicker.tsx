...
import { BUDDY_VOICE_PRESETS, clearBuddyVoiceClone, fileToVoiceDataUrl, getBuddyVoiceProfile, saveBuddyVoiceProfile, saveBuddyVoiceSample } from "@/lib/buddy-voice";
...
export function BuddyVoicePicker() {
  ...
  const preview = async () => {
    ...
    try {
      ...
      try { const result = await runStudioJob("tts", input, () => undefined); if (result.url) { const audio = new Audio(result.url); await audio.play(); setStatus("Buddy's selected voice is working."); return; } } catch { /* use browser fallback */ }
      browserPreview(text); setStatus("Remote voice engine unavailable; device voice fallback is playing.");
