...
const LANGUAGES = [
  "Auto", "English", "Spanish", "French", "German", "Italian", "Portuguese", "Dutch", "Danish", "Finnish", "Greek", "Swedish", "Norwegian", "Polish", "Russian", "Arabic", "Hebrew", "Hindi", "Chinese", "Cantonese", "Japanese", "Korean", "Malay", "Swahili", "Thai", "Turkish", "Vietnamese", "Tagalog", "Romanian", "Hungarian", "Persian (Farsi)", "Macedonian", "Czech",
];

export function VoiceProfilePanel() {
  const [voice, setVoice] = useState(localStorage.getItem("buddy-voice-choice") || "Ryan");
  const [language, setLanguage] = useState(localStorage.getItem("buddy-language") || "English");
  ...
  const saveSample = async (blob: Blob) => {
    ...
    setStatus("Your voice sample is saved on this device. Buddy can use it for authorized voice cloning.");
  };
  ...
  const previewVoice = async () => {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : "No verified voice route could complete the preview.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <Panel eyebrow="BUDDY • VOICE" title="Buddy's voice" icon={<Volume2 className="size-5" />} defaultOpen>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        ...
        <button type="button" onClick={() => void recordSample()} className="flex min-h-12 items-center gap-2 rounded-xl border border-border bg-background/50 px-3 text-left text-xs font-semibold">
          <Mic2 className="size-4 text-primary" /> {recording ? "Stop recording" : "Record my voice in Studio"}
        </button>
      </div>
