...
import { Brain, LoaderCircle, Mic, MicOff, Phone, Send, Sparkles, Volume2, VolumeX } from "lucide-react";
...
export function BuddyLiveChat() {
  ...
  async function beginCapture(mode: CaptureMode) {
    ...
    recorder.start(200);
    setRecording(true);
    setBuddyStatus("listening", { message: mode === "live" ? "Buddy is listening…" : "Recording your voice…" });
  ...
  async function answer(text: string, speakReply = false) {
    ...
    try {
      const prompt = [
        { role: "system" as const, content: "You are Buddy from Little Red's Big Studio: a sharp, warm, funny creative partner for music and YouTube. Never claim an action happened unless the Studio returned a verified result. Be concise and useful." },
        ...next,
      ];
    ...
    <Panel eyebrow="BUDDY • LIVE" title="Call Buddy" icon={<Sparkles className="size-5" />} defaultOpen>
      <div className="relative overflow-hidden rounded-[1.6rem] border border-primary/30 bg-[radial-gradient(circle_at_50%_25%,oklch(0.34_0.12_25_/_0.65),transparent_48%),linear-gradient(145deg,oklch(0.08_0.02_20),oklch(0.14_0.04_20))] p-5 shadow-[0_24px_70px_oklch(0_0_0_/_0.45)] sm:p-7">
        ...
        <div className="relative flex flex-col items-center text-center">
          ...
          <h3 className="mt-2 font-display text-xl font-black sm:text-2xl">{live ? "Buddy is with you" : "Talk naturally with Buddy"}</h3>
          <p className="mt-1 max-w-md text-xs leading-5 text-muted-foreground">One tap to start. Speak, pause, and Buddy answers with voice, then listens again.</p>
          <div className="mt-5 flex w-full max-w-md flex-col gap-2 sm:flex-row">
            <StudioButton className="min-h-12 flex-1" onClick={() => void toggleLive()} aria-pressed={live}>
              {live ? <Phone className="size-4" /> : <Phone className="size-4" />}
              {live ? "End Buddy Call" : "Call Buddy"}
            </StudioButton>
            <button type="button" onClick={() => { setMuted((value) => !value); window.speechSynthesis?.cancel(); }} className="min-h-12 rounded-xl border border-border bg-background/50 px-4 text-xs font-semibold">
      ...
      {micOptions.length > 1 && (
        <label className="mt-3 block rounded-xl border border-border bg-background/50 p-3 text-xs">
          ...
          <select value={micId} onChange={(event) => setMicId(event.target.value)} className="w-full rounded-lg border border-border bg-background px-2 py-2 text-sm">
            {micOptions.map((device) => <option key={device.id} value={device.id}>{device.label}</option>)}
          </select>
        </label>
      )}

      <div className="mt-3 flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs text-muted-foreground" aria-live="polite">
        {busy ? <LoaderCircle className="size-4 shrink-0 animate-spin text-primary" /> : live ? <Mic className="size-4 shrink-0 text-primary" /> : <Brain className="size-4 shrink-0 text-primary" />}
        <span>{status}</span>
      </div>

      <div className="mt-3 flex gap-2">
        <button type="button" disabled={busy || live} onClick={() => void beginCapture("record")} className="min-h-11 rounded-xl border border-border bg-background/50 px-3 text-xs font-semibold disabled:opacity-45">
          <Mic className="mr-2 inline size-4 text-primary" /> Record → Text
        </button>
        {recording && !live && <button type="button" onClick={stopCapture} className="min-h-11 rounded-xl border border-primary/40 bg-primary/10 px-3 text-xs font-semibold"><MicOff className="mr-2 inline size-4" /> Stop recording</button>}
      </div>

      {transcript && (
        <div className="mt-3 rounded-xl border border-border bg-background/40 p-3">
          <label className="text-xs font-semibold">Transcription</label>
          <textarea value={transcript} onChange={(event) => setTranscript(event.target.value)} rows={4} className="mt-2 w-full rounded-lg border border-border bg-background/60 p-3 text-sm" />
          <StudioButton className="mt-2" disabled={busy || !transcript.trim()} onClick={() => void answer(transcript, true)}><Send className="size-4" /> Send to Buddy</StudioButton>
        </div>
      )}
      ...
      <div className="mt-3 max-h-72 space-y-2 overflow-y-auto rounded-xl border border-border bg-background/35 p-3" aria-live="polite">
        {messages.length === 0 ? <p className="text-sm text-muted-foreground">“Alright. What are we making?”</p> : messages.map((message, index) => <div key={`${message.role}-${index}`} className={`max-w-[90%] whitespace-pre-wrap rounded-xl px-3 py-2 text-sm ${message.role === "user" ? "ml-auto crimson-gloss text-primary-foreground" : "border border-border bg-background/60"}`}>{message.content}</div>)}
      </div>

      <form className="mt-3 flex gap-2" onSubmit={(event) => { event.preventDefault(); void answer(input, false); }}>
        <input value={input} onChange={(event) => setInput(event.target.value)} disabled={busy} placeholder="Type to Buddy…" className="min-w-0 flex-1 rounded-xl border border-border bg-background/60 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring" />
        <StudioButton type="submit" disabled={busy || !input.trim()} aria-label="Send message"><Send className="size-4" /></StudioButton>
      </form>
      <p className="mt-2 text-[0.65rem] leading-4 text-muted-foreground">Buddy first requests permission, discovers available inputs, prefers a working non-default microphone, and falls back to the device default when necessary. A browser or OS denial cannot be bypassed.</p>
    </Panel>
  );
}
