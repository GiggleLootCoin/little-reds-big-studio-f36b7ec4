...
export function BuddyLiveChat() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      stopAll();
      attachments.forEach(revokeBuddyAttachment);
    };
  ...
  async function openMicrophone() {
    try {
      ...
      if (!track || track.readyState !== "live") {
        ...
        throw new Error("Android did not provide a live microphone stream.");
      }
    ...
    recorder.onstop = () => {
      ...
      if (captureMode === "record") {
        ...
        void transcribeBlob(blob)
          .then(setTranscript)
          .then(() => setStatus("Transcription ready. Edit it or send it to Buddy."))
          .catch((error) =>
            setStatus(error instanceof Error ? error.message : "Speech recognition failed."),
          );
      } else if (captureMode === "live" && liveRef.current && blob.size) {
        void transcribeBlob(blob)
          .then((text) => answer(text, true))
          .catch((error) => {
            setStatus(error instanceof Error ? error.message : "Speech recognition failed.");
            if (liveRef.current) setTimeout(() => void beginCapture("live"), 500);
          });
      } else {
        stopMicrophone(streamRef.current);
        streamRef.current = null;
  ...
  function startBrowserRecognition(mode: Mode) {
    ...
    modeRef.current = mode;
    recognitionRestartRef.current = mode === "live";
    ...
    recognition.onresult = (event) => {
      ...
      if (mode === "record")
        setTranscript(
          `${recognitionTextRef.current}${interimText ? ` ${interimText}` : ""}`.trim(),
        );
    };
    recognition.onerror = (event) => {
      ...
      if (error === "not-allowed" || error === "service-not-allowed") {
        recognitionRestartRef.current = false;
        setStatus(
          "Microphone permission was denied. Allow microphone access for this site, then tap Call Buddy again.",
        );
      } else if (error !== "aborted" && !speakingRef.current)
        setStatus("Browser speech recognition is unavailable; use the Studio speech engine.");
    };
    recognition.onend = () => {
      ...
      if (text) {
        ...
        } else if (liveRef.current) void answer(text, true);
      }
      if (
        liveRef.current &&
        mode === "live" &&
        ...
        setTimeout(() => {
          if (liveRef.current && !speakingRef.current) void beginCapture("live");
        }, 250);
      else if (!liveRef.current && mode !== "record")
        setStatus(text ? "Buddy received you." : "I didn't catch that. Try again.");
    };
    try {
      recognition.start();
      return true;
  ...
  async function beginCapture(mode: Mode) {
    ...
    if (mode === "record") {
      if (startBrowserRecognition("record")) return;
      startMediaRecorder("record", stream);
      return;
    }
  }
  ...
  async function speak(text: string) {
    ...
    try {
      ...
      audio.onended = () => {
        ...
        setBuddyStatus("idle");
        if (liveRef.current) setTimeout(() => void beginCapture("live"), 200);
      };
      audio.onerror = () => {
        speakingRef.current = false;
        if (liveRef.current) setTimeout(() => void beginCapture("live"), 200);
  ...
  async function answer(text: string, speakReply = false) {
    ...
    const userMessage: Message = {
      ...
      createdAt: Date.now(),
      attachments: currentAttachments.map(({ file: _file, url, ...metadata }) => ({
        ...metadata,
        url,
      })),
    };
      ...
      const imageParts = await Promise.all(
        currentAttachments
          ...
          .map(async (a) => ({
            ...
            image_url: { url: await fileToDataUrl(a.file) },
          })),
      );
      ...
      const prompt: Array<{ role: "system" | "user" | "assistant"; content: unknown }> = [
        {
          role: "system",
          content:
            "You are Buddy from Little Red's Big Studio, a warm, sharp creative partner for music and YouTube. Use the conversation context. Never claim a generation happened unless the Studio verified the artifact. If attachments are supplied, analyze them when possible.",
        },
        ...next.map((m) => ({ role: m.role, content: m.content })),
      ];
      if (imageParts.length)
        prompt[prompt.length - 1].content = [
          { type: "text", text: `${clean || "Analyze these attachments."}${attachmentText}` },
          ...imageParts,
        ];
      else if (attachmentText)
        prompt[prompt.length - 1].content =
          `${clean || "Analyze these attachments."}${attachmentText}`;
      const result = await runStudioJob(
        "chat",
        { prompt, text: clean || "Analyze the attached files.", messages: prompt },
        setStatus,
      );
  ...
  async function toggleLive() {
    if (liveRef.current) {
      stopAll();
      setLive(false);
      setStatus("Buddy call ended. Your conversation is still here.");
      return;
    }
    setLive(true);
  ...
  function addFiles(fileList: FileList | null) {
    ...
    for (const file of Array.from(fileList)) {
      try {
        added.push(acceptBuddyFile(file));
      } catch (error) {
        setStatus(error instanceof Error ? error.message : "Attachment could not be added.");
      }
    }
