...
async function uploadReference(referenceAudio: Blob, env: Env): Promise<string> {
  ...
  const response = await withTimeout(fetch(`${CHATTERBOX_SPACE}/gradio_api/upload`, {
    method: "POST", headers: authHeaders(env), body: form,
  }), 30000, "Uploading reference voice");
  if (!response.ok) {
...
function parseSse(text: string): { event: string; data: unknown }[] {
  return text.split(/\r?\n\r?\n/).map((block) => {
    ...
    if (raw && raw !== "null") { try { data = JSON.parse(raw); } catch { data = raw; } }
    return { event, data };
...
function findAudioArtifact(value: unknown): string | null {
  ...
  if (Array.isArray(value)) {
    for (const item of value) { const found = findAudioArtifact(item); if (found) return found; }
  }
  if (value && typeof value === "object") {
    ...
    for (const key of ["data", "value", "output", "result"]) {
      if (object[key] !== undefined) { const found = findAudioArtifact(object[key]); if (found) return found; }
    }
  }
  return null;
}
...
async function runChatterboxClone(referenceAudio: Blob, text: string, env: Env): Promise<Response> {
  ...
  const referenceFile = { path: referencePath, orig_name: "reference.wav", meta: { _type: "gradio.FileData" } };

  const start = await withTimeout(fetch(`${CHATTERBOX_SPACE}/gradio_api/call/generate_tts_audio`, {
    method: "POST",
    headers: { ...authHeaders(env), "content-type": "application/json" },
    body: JSON.stringify({ data: [text.slice(0, 300), referenceFile, 0.5, 0.8, 0, 0.5, false] }),
  }), 30000, "Starting Chatterbox clone");
  if (!start.ok) {
  ...
  const result = await withTimeout(fetch(`${CHATTERBOX_SPACE}/gradio_api/call/generate_tts_audio/${encodeURIComponent(started.event_id)}`, {
    headers: authHeaders(env),
  }), 180000, "Chatterbox voice clone generation");
  if (!result.ok) {
  ...
  const absolute = artifact.startsWith("http") ? artifact : `${CHATTERBOX_SPACE}/gradio_api/file=${artifact.replace(/^\/+/, "")}`;
  const generatedAudio = await withTimeout(fetch(absolute, { headers: authHeaders(env) }), 30000, "Downloading cloned audio");
  if (!generatedAudio.ok || !generatedAudio.body) {
    const detail = await generatedAudio.text().catch(() => "");
    throw new Error(`Chatterbox returned unusable audio (${generatedAudio.status})${detail ? `: ${detail.slice(0, 300)}` : ""}`);
  }
...
async function handleVoiceClone(request: Request, env: Env): Promise<Response> {
  ...
  catch { return Response.json({ ok: false, error: "The clone request was not valid JSON." }, { status: 400 }); }
  if (!body.audioBase64) return Response.json({ ok: false, error: "A voice sample is required." }, { status: 400 });
  try {
    ...
    return Response.json({ ok: false, error: error instanceof Error ? error.message : "Voice cloning failed." }, {
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (url.pathname === "/api/ai/voice-clone" && request.method === "POST") return handleVoiceClone(request, env);
