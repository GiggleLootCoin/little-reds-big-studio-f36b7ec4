...
const required = [
  ...
  ["clone encodes the actual reference recording", files.clone, "cachedReferenceBase64 = await blobBase64(sample)"],
  ...
  ["gateway reconstructs the actual reference Blob", files.gateway, "new Blob([decodeBase64(audioBase64)]"],
  ["gateway uploads through the Qwen file endpoint", files.gateway, "const form = new FormData()"],
  ["gateway uploads the reference to the selected Space", files.gateway, "fetch(`${space}/gradio_api/upload`"],
  ["gateway uses the Qwen voice-clone operation", files.gateway, "generate_voice_clone"],
  ["gateway uses full reference conditioning", files.gateway, "[file, refText, text, languageName(language), false, modelSize]"],
  ["gateway supports the fast and quality Qwen models", files.gateway, 'modelSize?: "0.6B" | "1.7B"'],
  ["gateway sends a Gradio FileData reference object", files.gateway, 'meta: { _type: "gradio.FileData" }'],
  ...
  ["gateway identifies the selected Qwen provider", files.gateway, 'headers.set("x-clone-provider", provider)'],
  ["gateway has a second free Qwen route", files.gateway, "QWEN_TTS_FALLBACK_SPACE_URL"],
  ["gateway uses the fallback Qwen voice-clone operation", files.gateway, 'officialClone(fallback, path'],
  ["gateway preserves the reference separately per Space", files.gateway, "const key = `${space}|${referenceId}`"],
  ["gateway rejects a completed response with no audio", files.gateway, "Qwen completed without an audio artifact"],
...
const forbiddenProductionRemote = ["rahul7star", "spacekaren", "/api/ai/voice-clone", "OPENROUTERAI_API_KEY"];
for (const [name, source] of Object.entries({ clone: files.clone, runtime: files.runtime })) {
  for (const needle of forbiddenProductionRemote) {
    if (source.includes(needle)) throw new Error(`Voice-path verification failed: forbidden legacy remote/API dependency found in ${name}: ${needle}`);
  }
}
...
if (!voiceBlock.includes("fallbackIds: []")) throw new Error("Voice capability registry must not advertise a legacy voice fallback.");

console.log("Buddy voice path verified: actual reference Blob -> content-hashed cached base64 -> /api/voice-clone -> Qwen3-TTS selected Base model with full reference conditioning -> free Qwen fallback -> returned Blob -> normalizeAndVerifyBrowserAudio() -> validated browser URL.");
