...
const required = [
  ...
  [
    "worker uses the Transformers.js Tensor contract",
    files.worker,
    'new Tensor("float32", audio, [1, audio.length])',
  ],
  ["worker performs speaker encoding", files.worker, "model.encode_speech(reference)"],
  [
    "worker validates all four conditioning outputs",
    files.worker,
    "assertConditioning(await model.encode_speech(reference))",
  ],
  ...
  [
    "worker uses the supported WebGPU language-model dtype",
    files.worker,
    'language_model: "q4f16"',
  ],
  [
    "worker rejects empty generated audio",
    files.worker,
    "Chatterbox generation returned empty audio",
  ],
  ["Buddy sample persistence is local only", files.buddy, "putVoiceValue(SAMPLE_KEY"],
  [
    "voice clone is intercepted before generic runtime",
    files.runtime,
    'if (capability === "voice-clone")',
  ],
  ["verified clone uses local clone engine", files.runtime, "createBestFreeVoiceClone"],
  [
    "generic runtime is loaded only after voice handling",
    files.runtime,
    'import("./studio-runtime-impl")',
  ],
];
...
for (const [name, source] of Object.entries(files)) {
  for (const needle of forbiddenSource) {
    if (source.includes(needle)) {
      throw new Error(
        `Voice-path verification failed: forbidden remote/API dependency found in ${name}: ${needle}`,
      );
    }
  }
}
