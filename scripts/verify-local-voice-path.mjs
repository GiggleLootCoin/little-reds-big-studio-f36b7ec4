...
const required = [
  ...
  ["gateway uses Qwen 0.6B", files.gateway, 'model_size: "0.6B"'],
  ...
  ["gateway identifies the Qwen provider", files.gateway, 'headers.set("x-clone-provider", "Qwen3-TTS 0.6B Base")'],
...
console.log(
  "Buddy voice path verified: actual reference Blob + transcript -> /api/voice-clone -> Qwen3-TTS 0.6B full reference conditioning -> returned Blob -> normalizeAndVerifyBrowserAudio() -> validated normalized browser URL; obsolete local Chatterbox production path rejected.",
);
