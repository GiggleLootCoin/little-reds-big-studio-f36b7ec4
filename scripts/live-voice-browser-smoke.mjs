...
const sttResponse = await fetch(`${base}/api/ai/speech-to-text?android_smoke=1&ts=${Date.now()}`, {
  method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ audioBase64, language: "en" }),
});
if (!sttResponse.ok) {
...
const productionResponse = await fetch(`${base}/api/voice-clone?android_smoke=1&ts=${Date.now()}`, {
  ...
  body: JSON.stringify({ referenceId, audioBase64, audioType: "audio/wav", refText, text: "Hello. This is the live Android browser playback test.", language: "en", modelSize: "0.6B" }),
});
...
if (!/^audio\/(wav|wave)(?:;|$)/i.test(contentType) && !/^application\/octet-stream(?:;|$)/i.test(contentType)) throw new Error(`unexpected production MIME: ${contentType}`);
const productionBytes = Buffer.from(await productionResponse.arrayBuffer());
if (productionBytes.byteLength <= 4096) throw new Error(`returned audio is too small: ${productionBytes.byteLength} bytes`);
