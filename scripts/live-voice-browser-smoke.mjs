...
if (!sttResponse.ok)
  throw new Error(
    `production STT returned HTTP ${sttResponse.status}: ${(await sttResponse.text().catch(() => "")).slice(0, 300)}`,
  );
...
function assertPlayableMime(contentType, label) {
  if (
    !/^audio\/(wav|wave|mpeg|mp3|ogg|webm|flac)(?:;|$)/i.test(contentType) &&
...
if (!productionResponse.ok)
  throw new Error(
    `production transcript clone returned HTTP ${productionResponse.status}: ${(await productionResponse.text().catch(() => "")).slice(0, 300)}`,
  );
...
if (!defaultResponse.ok)
  throw new Error(
    `production default Red clone returned HTTP ${defaultResponse.status}: ${(await defaultResponse.text().catch(() => "")).slice(0, 300)}`,
  );
