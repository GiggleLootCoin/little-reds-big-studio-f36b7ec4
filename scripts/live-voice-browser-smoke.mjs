...
if (!sttResponse.ok) throw new Error(`production STT returned HTTP ${sttResponse.status}: ${(await sttResponse.text().catch(() => "")).slice(0, 300)}`);
...
async function cloneRequest(extra = {}) {
  return fetch(`${base}/api/voice-clone?android_smoke=1&ts=${Date.now()}`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ referenceId, audioBase64, audioType: "audio/wav", text: "Hello. This is the live Android browser playback test.", language: "en", modelSize: "0.6B", ...extra }),
  });
}
function assertVoxCPMResponse(response, label) {
  ...
  if (provider !== "VoxCPM2 reference clone") throw new Error(`${label} used unexpected clone provider: ${provider || "missing"}`);
  if (route !== "voxcpm2-reference-clone") throw new Error(`${label} used unexpected voice route: ${route || "missing"}`);
}
function assertPlayableMime(contentType, label) {
  if (!/^audio\/(wav|wave|mpeg|mp3|ogg|webm|flac)(?:;|$)/i.test(contentType) && !/^application\/octet-stream(?:;|$)/i.test(contentType))
    throw new Error(`unexpected ${label} MIME: ${contentType}`);
}

const productionResponse = await cloneRequest({ refText });
if (!productionResponse.ok) throw new Error(`production transcript clone returned HTTP ${productionResponse.status}: ${(await productionResponse.text().catch(() => "")).slice(0, 300)}`);
...
assertPlayableMime(contentType, "production transcript-clone");
const productionBytes = Buffer.from(await productionResponse.arrayBuffer());
if (productionBytes.byteLength <= 4096) throw new Error(`returned transcript-clone audio is too small: ${productionBytes.byteLength} bytes`);

const defaultResponse = await cloneRequest({});
if (!defaultResponse.ok) throw new Error(`production default Red clone returned HTTP ${defaultResponse.status}: ${(await defaultResponse.text().catch(() => "")).slice(0, 300)}`);
...
assertPlayableMime(defaultContentType, "production default-Red");
const defaultBytes = Buffer.from(await defaultResponse.arrayBuffer());
if (defaultBytes.byteLength <= 4096) throw new Error(`returned default-Red audio is too small: ${defaultBytes.byteLength} bytes`);
if (createHash("sha256").update(defaultBytes).digest("hex") === referenceId) throw new Error("default Red clone returned the reference audio unchanged instead of generated speech");

const browser = await chromium.launch({ headless: true, args: ["--autoplay-policy=no-user-gesture-required"] });
let context;
try {
  context = await browser.newContext({ viewport: { width: 393, height: 852 }, deviceScaleFactor: 2.75, isMobile: true, hasTouch: true, userAgent: "Mozilla/5.0 (Linux; Android 12; Pixel 5) AppleWebKit/537.36 Chrome/140.0 Mobile Safari/537.36" });
  const page = await context.newPage();
  await page.goto(`${base}/?android_smoke=1`, { waitUntil: "networkidle", timeout: 60000 });
  const playback = await page.evaluate(async ({ productionBytes, contentType }) => {
    ...
    try {
      ...
      let peak = 0; let sumSquares = 0; let count = 0;
      for (let channel = 0; channel < decoded.numberOfChannels; channel++) {
        const samples = decoded.getChannelData(channel); const step = Math.max(1, Math.floor(samples.length / 200000));
        for (let i = 0; i < samples.length; i += step) { const sample = samples[i]; peak = Math.max(peak, Math.abs(sample)); sumSquares += sample * sample; count++; }
      }
  ...
  console.log(JSON.stringify({ status: "ok", referenceTranscript: refText, transcriptCloneBytes: productionBytes.byteLength, transcriptCloneProvider: productionResponse.headers.get("x-clone-provider"), defaultRedCloneBytes: defaultBytes.byteLength, defaultRedProvider: defaultResponse.headers.get("x-clone-provider"), androidPlayback: playback }, null, 2));
} catch (error) {
