...
if (!/^audio\/(wav|wave)(?:;|$)/i.test(contentType) && !/^application\/octet-stream(?:;|$)/i.test(contentType)) throw new Error(`unexpected production transcript-clone MIME: ${contentType}`);
...
if (!/^audio\/(wav|wave)(?:;|$)/i.test(defaultContentType) && !/^application\/octet-stream(?:;|$)/i.test(defaultContentType)) throw new Error(`unexpected production default-Red MIME: ${defaultContentType}`);
...
try {
  ...
  const playback = await page.evaluate(async ({ productionBytes, contentType }) => {
    ...
    try {
      ...
      await new Promise((resolve, reject) => { const timer = setTimeout(() => reject(new Error("HTMLAudioElement metadata timeout")), 10000); audio.onloadedmetadata = () => { clearTimeout(timer); resolve(); }; audio.onerror = () => { clearTimeout(timer); reject(new Error("HTMLAudioElement could not decode production Blob URL")); }; audio.load(); });
      ...
      audioContext.close();
      return { contentType, bytes: bytes.byteLength, duration: decoded.duration, peak, rms, htmlAudioDuration: audio.duration, readyState: audio.readyState, paused: audio.paused, playCalls: calls };
    } finally { URL.revokeObjectURL(url); }
  }, { productionBytes: [...defaultBytes], contentType: defaultContentType });
  console.log(JSON.stringify({ status: "ok", referenceTranscript: refText, transcriptCloneBytes: productionBytes.byteLength, defaultRedCloneBytes: defaultBytes.byteLength, defaultRedProvider: defaultResponse.headers.get("x-clone-provider") || "unknown", androidPlayback: playback }, null, 2));
} catch (error) {
