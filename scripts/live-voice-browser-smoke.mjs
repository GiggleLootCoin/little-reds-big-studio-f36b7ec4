...
const productionResponse = await fetch(`${base}/api/ai/voice-clone?android_smoke=1&ts=${Date.now()}`, { method: "POST", body: form });
if (!productionResponse.ok) throw new Error(`production clone returned HTTP ${productionResponse.status}`);
...
try {
  ...
  const playback = await page.evaluate(async ({ productionBytes, contentType, cors }) => {
