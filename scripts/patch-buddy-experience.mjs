...
replaceRegex(
  chatPath,
  /\s+const history = \[\{ role: "system", content: IDENTITY \}, \.\.\.prior, \{ role: "user", content \}\];\s+const r = await runStudioJob\(\s+"chat",\s+\{ prompt: clean, text: clean, messages: history, history \},/,
...
replaceRegex(
  chatPath,
  /language: v\.language \|\| "English",\s+use_xvector_only: !v\.referenceTranscript,/,
  'language: v.language || "English",\n            mood: v.mood || "natural",\n            tone: v.tone || "conversational",\n            use_xvector_only: !v.referenceTranscript,',
  "forward mood tone into Red voice generation",
);
replaceRegex(
  chatPath,
  /"tts",\s+\{ text, target_text: text, language: v\.language \|\| "English", speaker: v\.speaker \},/,
...
replaceRegex(
  agentPath,
  /\s+"Return a concise natural reply plus a machine-readable action plan when creation work is requested\.",/,
