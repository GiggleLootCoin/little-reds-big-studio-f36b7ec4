...
for (const [file, source] of reachableAssets) {
  if (source.includes("/api/voice-clone"))
    throw new Error(
      `Production browser graph contains the obsolete /api/voice-clone endpoint in ${file}.`,
    );
...
console.log(
  "Compiled browser graph verified: production clone entry → /api/ai/voice-clone; obsolete endpoint and local clone path are absent.",
);
